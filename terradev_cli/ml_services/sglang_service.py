...
class SGLangOptimizer:
    ...
    def _init_model_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            # Dense models
            "llama": {"type": "dense", "workload_type": WorkloadType.AGENTIC_CHAT},
            "mistral": {"type": "dense", "workload_type": WorkloadType.AGENTIC_CHAT},
            "qwen": {"type": "dense", "workload_type": WorkloadType.AGENTIC_CHAT},
            "glm": {"type": "dense", "workload_type": WorkloadType.AGENTIC_CHAT},
        }
...
class SGLangService:
    ...
    def detect_workload_type(
        self, model_path: str, user_description: Optional[str] = None
    ) -> WorkloadType:
        ...
        if user_description:
            ...
            if any(
                term in desc_lower
                for term in ["agentic", "multi-turn", "chat", "conversation"]
            ):
                return WorkloadType.AGENTIC_CHAT
            elif any(
                term in desc_lower
                for term in ["batch", "eval", "dataset", "processing"]
            ):
                return WorkloadType.BATCH_INFERENCE
            elif any(
                term in desc_lower for term in ["latency", "real-time", "ttft", "tpot"]
            ):
                ...
                term in desc_lower
                for term in ["disaggregated", "prefill", "decode", "pd"]
            ):
    ...
    def optimize_for_workload(
        self, config: SGLangConfig, hardware: HardwareProfile
    ) -> SGLangConfig:
        """Apply workload-specific optimizations"""
    ...
    def _apply_moe_parallelism(
        self, config: SGLangConfig, model_type: str, hardware: HardwareProfile
    ) -> SGLangConfig:
    ...
    def _optimize_pd_disaggregated(
        self, config: SGLangConfig, hardware: HardwareProfile
    ) -> SGLangConfig:
    ...
    def _optimize_structured_output(
        self, config: SGLangConfig, hardware: HardwareProfile
    ) -> SGLangConfig:
    ...
    def generate_launch_command(self, config: SGLangConfig) -> str:
        ...
        if config.cuda_graph_bs:
            cmd_parts.extend(
                ["--cuda-graph-bs"] + [str(bs) for bs in config.cuda_graph_bs]
            )
        ...
        if config.speculative_algorithm:
            cmd_parts.extend(
                ["--speculative-algorithm", config.speculative_algorithm.value]
            )
            cmd_parts.extend(
                ["--speculative-num-steps", str(config.speculative_num_steps)]
            )
            cmd_parts.extend(
                ["--speculative-eagle-topk", str(config.speculative_eagle_topk)]
            )
            cmd_parts.extend(
                [
        ...
        if config.ep_num_redundant_experts > 0:
            cmd_parts.extend(
                ["--ep-num-redundant-experts", str(config.ep_num_redundant_experts)]
            )
        ...
        if config.disaggregation_ib_device:
            cmd_parts.extend(
                ["--disaggregation-ib-device", config.disaggregation_ib_device]
            )
    ...
    def validate_config(self, config: SGLangConfig) -> List[str]:
        ...
        if (
            config.attention_backend == AttentionBackend.FA3
            and not hardware.supports_fa3
        ):
            warnings.append(
                f"FA3 attention backend not supported on {hardware.gpu_type}"
            )

        # Workload-specific checks
        if (
            config.workload_type == WorkloadType.LOW_LATENCY
            and config.max_running_requests > 128
        ):
            warnings.append("High max-running-requests may impact latency SLA")

        if (
            config.workload_type == WorkloadType.MOE_MODEL
            and config.ep_num_redundant_experts > 32
        ):
        ...
        if config.deepep_mode == DeepEPMode.AUTO and config.disaggregation_mode:
            warnings.append(
                "Consider using specific DeepEP mode for disaggregated serving"
            )
    ...
    async def start_server(
        ...
        try:
            ...
            if self.config.max_model_len:
                server_cmd.extend(
                    ["--max-total-tokens", str(self.config.max_model_len)]
                )
...
"""
    ...
    async def get_server_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(
                metrics_url, timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                if response.status == 200:
...
def create_sglang_service_from_credentials(
    ...
    config = SGLangConfig(
        ...
        max_model_len=(
            int(credentials["max_model_len"])
            if credentials.get("max_model_len")
            else None
        ),
        trust_remote_code=credentials.get("trust_remote_code", "true").lower()
        == "true",
        enable_expert_parallel=credentials.get(
