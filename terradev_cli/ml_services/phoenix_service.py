...
class PhoenixService:
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logger.warning(
                        "Phoenix %s %s error: %s, retry in %.1fs", method, path, e, wait
                    )
                    await asyncio.sleep(wait)
                    continue
                raise
    ...
    async def list_spans(
        ...
        *,
        ...
        return await self._request(
            "GET", f"/v1/projects/{project}/spans", params=params
        )

    async def get_trace(
        self, trace_id: str, project_identifier: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def generate_instrumentation_snippet(
...
def create_phoenix_service_from_credentials(
    credentials: Dict[str, str]
) -> PhoenixService:
    config = PhoenixConfig(
        collector_endpoint=credentials.get(
