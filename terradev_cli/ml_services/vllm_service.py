...
class VLLMConfig:
    ...
    def create_auto_optimized(
        cls, model_name: str, workload: WorkloadProfile, **kwargs
    ) -> "VLLMConfig":
        ...
        base_seqs = max(
            workload.concurrent_users, int(workload.requests_per_second * 2)
        )
        ...
        enable_prefix_cache = (
            workload.avg_prompt_length > 100  # Long prompts likely have shared prefixes
            or workload.concurrent_users > 5  # Multi-user scenarios
            or workload.requests_per_second > 5  # High QPS benefits from caching
        )

        # Determine chunked prefill
        # Most beneficial for long prompts and high QPS
        enable_chunked_prefill = (
            workload.avg_prompt_length > 512  # Long prompts
            or workload.requests_per_second > 2  # Any significant QPS
        )
    ...
    @classmethod
    def _create_reasoning_optimized(
        cls, model_name: str, workload: WorkloadProfile, **kwargs
    ) -> "VLLMConfig":
    ...
    @classmethod
    def analyze_workload_from_samples(
        cls, samples: List[Dict[str, Any]], gpu_count: int = 1
    ) -> WorkloadProfile:
        ...
        concurrent_users = min(
            len(set(s.get("user_id", "default") for s in samples)), 10
        )
...
"""
    ...
    def _build_server_args(self) -> List[str]:
        ...
        if self.config.enable_lora:
            ...
            args.extend(
                ["--lora-extra-vocab-size", str(self.config.lora_extra_vocab_size)]
            )
            ...
            if self.config.lora_tuned_config_dir:
                args.extend(
                    ["--override-neuron-config", self.config.lora_tuned_config_dir]
                )
        ...
        if self.config.speculative_method:
            if self.config.speculative_method == "mtp":
                ...
                args.extend(
                    [
                        f"--speculative-config.num_speculative_tokens={self.config.num_speculative_tokens}"
                    ]
                )
                ...
                args.extend(
                    [
                        ...
                        str(self.config.num_speculative_tokens),
                    ]
                )
    ...
    async def wake(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Wake up
            async with self.session.post(
                f"{base}/wake_up", timeout=aiohttp.ClientTimeout(total=60)
            ) as resp:
                if resp.status != 200:
                    body = await resp.text()
                    return {
                        "status": "failed",
                        "error": f"wake_up HTTP {resp.status}: {body}",
                    }
    ...
    def get_deployment_script(
        ...
        if self.config.enable_flashinfer:
            env_lines += (
                f"Environment=VLLM_ATTENTION_BACKEND={self.config.attention_backend}\n"
            )
...
"""
    ...
    async def analyze_current_workload(
        ...
        try:
            ...
            async with self.session.get(
                ...
                if resp.status != 200:
                    return {
                        "status": "failed",
                        "error": f"Metrics endpoint not available: {resp.status}",
                    }
    ...
    async def auto_optimize_from_workload(
        self, samples: Optional[List[Dict[str, Any]]] = None, gpu_count: int = 1
    ) -> Dict[str, Any]:
    ...
    def _compare_configurations(
        self, current: "VLLMConfig", optimized: "VLLMConfig"
    ) -> List[Dict[str, Any]]:
            ...
            changes.append(
                {
                    ...
                    "impact": (
                        "throughput"
                        if optimized.max_num_batched_tokens
                        > current.max_num_batched_tokens
                        else "latency"
                    ),
                }
            )
