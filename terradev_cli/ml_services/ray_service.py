...
class RayService:
    ...
    async def get_cluster_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                status_info = {"status": "running", "details": status_lines}
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if "job" in line.lower() or "task" in line.lower():
                        job_info = {"info": line.strip(), "status": "running"}
        ...
        try:
            if head_node:
                ...
                cmd = [
                    ...
                    "start",
                    "--address",
                    f"{self.config.head_node_ip}:{port}",
                ]
    ...
    async def stop_cluster(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["ray", "stop"], capture_output=True, text=True, timeout=30
            )
    ...
    async def get_job_submission_command(
        self,
        script_path: str,
        job_name: Optional[str] = None,
        resources: Optional[Dict[str, Any]] = None,
