...
class DigitalOceanProvider(BaseProvider):
            ...
            quotes.append(
                {
                    ...
                    "memory_gb": (
                        (size.get("memory", 0) or 0) // 1024
                        if (size.get("memory", 0) or 0) > 1024
                        else size.get("memory", 0)
                    ),
                    "gpu_count": 1,
                    "spot": False,
                }
    ...
    async def provision_instance(
        ...
        droplet_id = str(
            droplet.get("id", f"do-{datetime.now().strftime('%Y%m%d%H%M%S')}")
        )
        return {
    ...
    async def get_instance_status(self, instance_id: str) -> Dict[str, Any]:
        if not self.api_key:
            raise Exception("DigitalOcean API token not configured")
        data = await self._make_request(
            "GET", f"{self.API_BASE}/droplets/{instance_id}"
        )
        droplet = data.get("droplet", data)
        public_ip = None
        ...
        return {
            ...
            "status": status_map.get(
                droplet.get("status", ""), droplet.get("status", "unknown")
            ),
            "provider": "digitalocean",
            "public_ip": public_ip,
        }
    ...
    async def stop_instance(self, instance_id: str) -> Dict[str, Any]:
        ...
        await self._make_request(
            "POST",
            f"{self.API_BASE}/droplets/{instance_id}/actions",
            json={"type": "shutdown"},
        )
    ...
    async def list_instances(self) -> List[Dict[str, Any]]:
        ...
        try:
            data = await self._make_request(
                "GET",
                f"{self.API_BASE}/droplets?tag_name=terradev",
            )
                ...
                instances.append(
                    {
                        ...
                        "instance_type": (
                            d.get("size", {}).get("slug", "unknown")
                            if isinstance(d.get("size"), dict)
                            else d.get("size_slug", "unknown")
                        ),
                        "region": (
                            d.get("region", {}).get("slug", "unknown")
