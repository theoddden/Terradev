...
class InferXProvider(BaseProvider):
    ...
    def __init__(self, credentials: Dict[str, str]):
        ...
        self.name = "inferx"
        self.api_endpoint = credentials.get("api_endpoint", "https://api.inferx.com")
    ...
    async def deploy_model(self, model_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            payload = {
                ...
                "snapshot_enabled": model_config.get(
                    "snapshot_enabled", self.snapshot_enabled
                ),
            ...
            async with session.post(
                f"{self.api_endpoint}/v1/models/deploy", json=payload
            ) as response:
    ...
    async def list_models(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with session.get(f"{self.api_endpoint}/v1/models") as response:
                if response.status == 200:
                    result = await response.json()
    ...
    async def update_model(
        ...
        try:
            ...
            async with session.patch(
                f"{self.api_endpoint}/v1/models/{deployment_id}", json=updates
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    return result
    ...
    async def get_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.get(f"{self.api_endpoint}/v1/usage") as response:
                if response.status == 200:
                    result = await response.json()
                    return {
