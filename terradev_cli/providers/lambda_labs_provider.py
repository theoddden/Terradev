...
class LambdaLabsProvider(BaseProvider):
    ...
    async def get_instance_quotes(
        ...
        try:
            ...
            if live:
                ...
                for quote in live:
                    capacity_check = await self._check_capacity_availability(
                        quote["instance_type"]
                    )
                    quote.update(
                        {
    ...
    async def _get_live_availability(self, gpu_type: str) -> List[Dict[str, Any]]:
        ...
        for type_name, type_data in data.get("data", {}).items():
            ...
            if gpu_type.lower() in gpu_desc.lower():
                price = (
                    type_data.get("instance_type", {}).get("price_cents_per_hour", 0)
                    / 100
                )
    ...
    async def provision_instance(
        ...
        if not capacity_check["available"]:
            ...
            raise Exception(
                f"Lambda capacity unavailable for {instance_type}: {capacity_check['reason']}"
            )
        ...
        try:
            ...
            return {
                "instance_id": (
                    ids[0]
                    if ids
                    else f"lambda-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ),
    ...
    async def start_instance(self, instance_id: str) -> Dict[str, Any]:
        raise Exception(
            "Lambda Labs does not support restart — launch a new instance instead"
        )
    ...
    async def _route_to_fallback(
        ...
        for provider, config in fallback_configs.items():
            ...
            score = config["price_per_hour"] + (
                config["egress_cost"] * 10
            )  # Weight egress costs
            if score < best_score:
                best_score = score
                best_fallback = {"provider": provider, "region": region, **config}
