...
class AzureProvider(BaseProvider):
    ...
    async def provision_instance(
        ...
        if not quota_check["available"]:
            raise Exception(
                f"GPU quota blocked: {quota_check['reason']}. {quota_check['action_required']}"
            )
    ...
    async def list_instances(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            vms = await loop.run_in_executor(
                None,
                lambda: list(
                    self.compute_client.virtual_machines.list(self.resource_group)
                ),
            )
            return [
                {
    ...
    async def _check_gpu_quota(self, gpu_type: str, region: str) -> Dict[str, Any]:
        ...
        try:
            ...
            available_quota = (
                sub_quota.properties.limit.value
                - sub_quota.properties.current_value.value
            )
            ...
            return {
                ...
                "current": sub_quota.properties.current_value.value,
                "ml_workspace_quota": (
                    ml_workspace_quotas.get("available")
                    if ml_workspace_quotas
                    else None
                ),
            }
    ...
    async def _check_ml_workspace_quota(
