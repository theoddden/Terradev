...
def register_hf_spaces_commands(cli, TerradevAPI):
    ...
    @click.option(
        "--sdk", default="gradio", type=click.Choice(["gradio", "streamlit", "docker"])
    )
    @click.option("--private", is_flag=True)
    @click.option(
        "--template", type=click.Choice(["llm", "embedding", "image"]), default=None
    )
    def create(space_name, model_id, hardware, sdk, private, template):
        ...
        if template:
            fn = {
                "llm": HFSpaceTemplates.get_llm_template,
                "embedding": HFSpaceTemplates.get_embedding_template,
                "image": HFSpaceTemplates.get_image_model_template,
            }[template]
    ...
    def list_spaces(author, limit, fmt):
        ...
        if fmt == "json":
            print(json.dumps(spaces[:limit], indent=2, default=str))
            return
    ...
    def hardware(space_id, new_hw):
        ...
        if new_hw:
            _hf_api(
                "POST", f"/spaces/{space_id}/hardware", token, body={"flavor": new_hw}
            )
