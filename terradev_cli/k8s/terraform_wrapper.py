...
class TerraformWrapper:
    ...
    def _run_terraform_command(
        ...
        self.logger.info(
            f"Running Terraform command: {' '.join(command)} in {work_dir}"
        )
    ...
    def create_cluster(self, cluster_config: Dict) -> bool:
        ...
        try:
            ...
            result = self._run_terraform_command(
                ["terraform", "apply", "cluster.plan", "-auto-approve"]
            )
    ...
    def destroy_cluster(self, cluster_name: str) -> bool:
        ...
        try:
            # Log telemetry
            self.telemetry.log_action(
                "k8s_cluster_destroy", {"cluster_name": cluster_name}
            )
    ...
    def get_cluster_info(self, cluster_name: str) -> Optional[Dict]:
        ...
        try:
            cluster_file = (
                Path.home() / ".terradev" / "clusters" / f"{cluster_name}.json"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Terradev Kubernetes Terraform Wrapper"
    )
    parser.add_argument(
        "command",
        choices=["create", "destroy", "list", "info"],
    ...
    parser.add_argument(
        "--multi-cloud", action="store_true", help="Use multi-cloud provisioning"
    )
    parser.add_argument(
        "--prefer-spot", action="store_true", help="Prefer spot instances"
    )
    parser.add_argument("--aws-region", help="AWS region")
    parser.add_argument("--gcp-region", help="GCP region")
    parser.add_argument(
        "--control-plane",
        choices=["eks", "gke", "self-hosted"],
    ...
    try:
        if args.command == "create":
            if not args.cluster or not args.gpu or not args.count:
                print(
                    "Error: --cluster, --gpu, and --count are required for create command"
                )
                sys.exit(1)
