...
class InferXK8sSetup:
        ...
        try:
            ns = client.V1Namespace(
                metadata=client.V1ObjectMeta(
                    name=namespace,
                    labels={"name": namespace, "platform": "inferx-serverless"},
                )
            )
        ...
        try:
            if api_version == "v1":
                if kind == "Namespace":
                    ...
                    self.v1.create_namespaced_limit_range(
                        namespace=namespace, body=manifest
                    )
    ...
    async def _get_loadbalancer_ip(
        ...
        try:
            if replicas:
                ...
                self.apps_v1.patch_namespaced_deployment_scale(
                    name="inferx-platform", namespace=namespace, body=body
                )
...
def deploy(
    namespace, gpu_nodes, gpu_type, snapshot, gpu_slicing, multi_tenant, kubeconfig
):
    """Deploy InferX platform on Kubernetes"""
...
def scale(namespace, replicas, gpu_nodes, kubeconfig):
    ...
    try:
        results = asyncio.run(
            setup.scale_platform(
                namespace=namespace, replicas=replicas, gpu_nodes=gpu_nodes
            )
        )
...
@click.confirmation_option(
    prompt="Are you sure you want to delete the InferX platform?"
)
