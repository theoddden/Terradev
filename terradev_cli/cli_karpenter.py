...
def register_karpenter_commands(cli, TerradevAPI):
    ...
    def install(ver, cluster_name):
        ...
        from ml_services.kubernetes_service import (
            create_kubernetes_service_from_credentials,
        )
        ...
        print(
            f"\u2705 Karpenter {r['version']} installed"
    ...
    def status(fmt):
        ...
        svc = create_enhanced_kubernetes_service_from_credentials(
            TerradevAPI()._provider_creds("kubernetes")
        )
        result = asyncio.run(svc.get_monitoring_status())
        if fmt == "json":
            print(json.dumps(result, indent=2, default=str))
            return
        karp = result.get("karpenter", {})
        k8s = result.get("kubernetes", {})
        print(f"\n  Karpenter: {karp.get('status', '?')}")
        print(
            f"  Nodes: {k8s.get('total_nodes', '?')} total, {k8s.get('gpu_nodes', '?')} GPU"
        )
    ...
    def nodepools(fmt):
        ...
        if fmt == "json":
            ...
            return
        if not items:
            print("  No NodePools found.")
            return
        for np_item in items:
            spec = np_item.get("spec", {})
            lim = (
                ", ".join(f"{k}={v}" for k, v in spec.get("limits", {}).items())
                or "none"
            )
            print(
                f"  {np_item['metadata']['name']:<24} {spec.get('disruption', {}).get('consolidationPolicy', '?'):<20} {lim}"
            )

    @karpenter.command("create-nodepool")
    @click.option(
        "--gpu-type", "-g", required=True, help="H100, A100, A10G, L40S, L4, T4, V100"
    )
    ...
    def create_nodepool(gpu_type, cpu_limit, memory_limit):
        ...
        creds = TerradevAPI()._provider_creds("kubernetes")
        creds["karpenter_enabled"] = "true"
        svc = create_kubernetes_service_from_credentials(creds)
        r = asyncio.run(
            svc.create_karpenter_provisioner(
                gpu_type, {"cpu": cpu_limit, "memory": memory_limit}
            )
        )
        if r["status"] == "created":
            t = r.get("topology", {})
            print(
                f"\u2705 NodePool '{r['provisioner']}' — {', '.join(t.get('instance_families', []))}, RDMA={t.get('gpudirect_rdma')}"
            )
        else:
            print(f"\u274c {r['error']}")
    ...
    def events(limit):
        ...
        for ev in json.loads(r.stdout).get("items", [])[-limit:]:
            ...
            print(
                f"  {ts}  {ev.get('type','?'):<8} {(ev.get('reason') or '?')[:20]:<22} {(ev.get('message') or '')[:60]}"
            )
    ...
    def gpu_nodes(fmt):
        ...
        svc = create_kubernetes_service_from_credentials(
            TerradevAPI()._provider_creds("kubernetes")
        )
        ...
        for n in nodes:
            print(
                f"  {n['name'][:34]:<36} {n['instance_type'][:18]:<20} {n['gpu_capacity']} GPUs  {n['status']}"
            )
