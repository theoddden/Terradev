...
class SmartDeploymentRouter:
    ...
    async def recommend_deployments(
    ...
    async def _generate_all_options(
        self, requirements: DeploymentRequirements
    ) -> List[DeploymentOption]:
        """Generate all possible deployment options"""
        options = []
        ...
        async with self.price_engine as engine:
            price_infos = await engine.get_realtime_prices(
                requirements.gpu_type, requirements.region
            )

        # Generate options for each strategy
        for strategy_type, strategy in self.deployment_strategies.items():
            strategy_options = await strategy.generate_options(
                requirements, price_infos
            )
    ...
    async def execute_deployment(
        self, chosen_option: DeploymentOption, requirements: DeploymentRequirements
    ) -> Dict[str, Any]:
...
class DirectDeploymentStrategy:
    ...
    async def generate_options(
        self, requirements: DeploymentRequirements, price_infos: List
    ) -> List[DeploymentOption]:
        ...
        for price_info in price_infos:
            ...
            total_cost = (
                price_info.price * requirements.gpu_count * requirements.estimated_hours
            )
    ...
    async def generate_options(
        ...
        for price_info in price_infos:
            ...
            option = DeploymentOption(
                ...
                estimated_total_cost=total_cost,
                confidence=price_info.confidence
                * 0.9,  # Slightly lower confidence for K8s
                ...
                cons=["Complex setup", "Higher overhead", "Steeper learning curve"],
                metadata={
                    "spot": price_info.spot,
                    "capacity": price_info.capacity,
                    "region": price_info.region,
