...
class EgressCostMonitor:
    ...
    async def analyze_egress_cost(
        ...
        if alert_level != EgressCostLevel.LOW:
            alert = EgressCostAlert(
                ...
                message=self._generate_alert_message(
                    alert_level, estimated_cost, data_size_gb
                ),
        ...
        return {
            ...
            "budget_remaining": (
                self.budget_limit - self._get_monthly_spend()
                if self.budget_limit
                else None
            ),
            "budget_exceeded": self.budget_limit
            and (self._get_monthly_spend() + estimated_cost) > self.budget_limit,
        }

    def _get_egress_cost(
        self, src_provider: str, src_region: str, dst_provider: str
    ) -> float:
    ...
    def _generate_alert_message(
        self, level: EgressCostLevel, cost: float, size_gb: float
    ) -> str:
    ...
    async def _generate_recommendations(
        ...
        if src_provider == dst_provider:
            recommendations.append(
                "Same-cloud transfer - use internal networking to minimize costs"
            )
        ...
        if (
            ...
            recommendations.append(
                "Consider using zero-egress providers for cost savings"
            )

        # Large data transfer recommendations
        if data_size_gb > 100:
            recommendations.append(
                "For large transfers (>100GB), consider using physical storage shipment"
            )
            recommendations.append(
                "Use compression to reduce data size before transfer"
            )

        # High cost recommendations
        if estimated_cost > 50:
            recommendations.append(
                "High egress cost detected - consider multi-hop routing"
            )
        ...
        if src_provider != dst_provider:
            recommendations.append(
                "Cross-cloud transfer detected - check for regional cost variations"
            )
    ...
    async def _find_alternative_routes(
        ...
        for provider in zero_egress_providers:
            if provider != src_provider and provider != dst_provider:
                ...
                cost_to_provider = (
                    self._get_egress_cost(src_provider, src_region, provider)
                    * data_size_gb
                )
                cost_from_provider = (
                    self._get_egress_cost(provider, "us-east-1", dst_provider)
                    * data_size_gb
                )
                ...
                alternatives.append(
                    {
                        ...
                        "savings": max(
                            0,
                            (
                                self._get_egress_cost(
                                    src_provider, src_region, dst_provider
                                )
                                * data_size_gb
                            )
                            - total_cost,
        ...
        if src_provider != dst_provider:
            ...
            same_region_cost = (
                self._get_egress_cost(src_provider, src_region, dst_provider)
                * data_size_gb
            )
                ...
                alternatives.append(
                    {
                        ...
                        "total_cost": same_region_cost
                        * 0.5,  # Assume 50% savings for same-region
    ...
    async def get_cost_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_costs = [
            record for record in self.cost_history if record["timestamp"] >= cutoff_date
        ]
        ...
        first_half_cost = sum(
            record["estimated_cost"] for record in recent_costs[:mid_point]
        )
        ...
        return {
            ...
            "top_routes": [
                {"route": route, "cost": round(cost, 2)} for route, cost in top_routes
            ],
            "cost_trend": trend,
            "budget_remaining": (
            ...
            "budget_utilization": (
                round((total_cost / self.budget_limit) * 100, 1)
                if self.budget_limit
                else None
            ),
        }
