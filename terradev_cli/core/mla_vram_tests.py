...
class MLA_VRAM_Tests:
    ...
    async def test_deepseek_v3_mla_accuracy(self) -> Dict[str, Any]:
        ...
        test_cases = [
            {
                "context": 4096,
                "expected_kv_gb": 0.61,
                "expected_total_gb": 1479.31,
                "expected_gpus": 18,
            },  # Large model needs many GPUs
            {
                "context": 8192,
                "expected_kv_gb": 1.22,
            ...
            },  # Similar GPU count
            {
                "context": 16384,
                "expected_kv_gb": 2.44,
                "expected_total_gb": 1481.14,
                "expected_gpus": 19,
            },  # Slightly more GPUs
            {
                "context": 32768,
                "expected_kv_gb": 4.88,
                "expected_total_gb": 1483.58,
                "expected_gpus": 19,
            },  # More GPUs
        ]
        ...
        for case in test_cases:
            ...
            kv_accurate = (
                abs(estimate.kv_cache_gb - case["expected_kv_gb"]) <= kv_tolerance
            )
            total_accurate = (
                abs(estimate.total_gb - case["expected_total_gb"]) <= total_tolerance
            )
    ...
    async def test_standard_vs_mla_comparison(self) -> Dict[str, Any]:
        ...
        comparison = self.estimator.compare_standard_vs_mla(
            model_id="deepseek-v3", context_tokens=16384, target_gpu_vram_gb=80.0
        )
        ...
        return {
            "test_name": test_name,
            "status": (
                "PASSED" if compression_adequate and savings_adequate else "FAILED"
            ),
    ...
    async def test_gpu_count_recommendations(self) -> Dict[str, Any]:
        ...
        test_cases = [
            ...
            },  # Large model needs many GPUs
            {
                ...
                "context": 32768,
                "expected_gpus": 19,
            },  # Slightly more with larger context
            {
                "model": "llama-3-70b",
                "context": 8192,
                "expected_gpus": 2,
            },  # Standard MHA needs fewer GPUs
        ]
    ...
    async def test_context_scaling(self) -> Dict[str, Any]:
        ...
        base_estimate = self.estimator.estimate_vram(
            model_id="deepseek-v3", context_tokens=4096, target_gpu_vram_gb=80.0
        )
        ...
        for case in edge_cases:
            try:
                ...
                if case["should_fail"]:
                    ...
                    results.append(
                        {"case": case, "failed_as_expected": False, "error": str(e)}
                    )
    ...
    async def test_model_registry(self) -> Dict[str, Any]:
        ...
        non_mla_detected = all(
            not self.estimator.is_mla_model(model) for model in non_mla_models
        )
...
async def main():
    ...
    comparison_test = next(
        (r for r in results["test_results"] if "Comparison" in r["test_name"]), None
    )
    if comparison_test and comparison_test["status"] == "PASSED":
        print(
            f"  ✅ MLA compression ratio validated: {comparison_test.get('kv_compression_ratio', 'N/A')}x"
        )
