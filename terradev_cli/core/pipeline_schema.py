...
class TerradevAnnotations:
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        if self.migration_enabled is not None:
            annotations["terradev.io/migration-enabled"] = str(
    ...
    def from_dict(cls, annotations: Dict[str, str]) -> "TerradevAnnotations":
        ...
        if "terradev.io/migration-enabled" in annotations:
            terradev.migration_enabled = (
                annotations["terradev.io/migration-enabled"].lower() == "true"
            )
...
class Workflow:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.spec:
            spec_dict = {"entrypoint": self.spec.entrypoint, "templates": []}
            ...
            for template in self.spec.templates:
                ...
                if template.container:
                    container_dict = {"image": template.container.image}
                    ...
                    if template.container.volume_mounts:
                        container_dict["volumeMounts"] = (
                            template.container.volume_mounts
                        )
                ...
                if template.metadata:
                    template_dict["metadata"] = {"name": template.metadata.name}
    ...
    def validate_workflow(workflow: Workflow) -> List[str]:
        ...
        if workflow.spec and workflow.spec.entrypoint and workflow.spec.templates:
            ...
            if workflow.spec.entrypoint not in template_names:
                errors.append(
                    f"Entrypoint template '{workflow.spec.entrypoint}' not found in templates"
                )
        ...
        if workflow.metadata:
            ...
            if terradev_ann.gpu_count and (
                terradev_ann.gpu_count < 1 or terradev_ann.gpu_count > 32
            ):
                errors.append("GPU count must be between 1 and 32")

            if terradev_ann.tier and terradev_ann.tier not in [
                "research",
                "research+",
                "enterprise",
            ]:
            ...
            if (
                terradev_ann.cost_optimization
                and terradev_ann.cost_optimization
                not in ["conservative", "moderate", "aggressive"]
            ):
                errors.append(
                    "Cost optimization must be one of: conservative, moderate, aggressive"
                )
