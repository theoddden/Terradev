...
class KVCacheCheckpointManager:
    ...
    def __init__(self, config: CheckpointConfig):
        ...
        self.active_checkpoints: Dict[str, KVCacheCheckpoint] = (
            {}
        ...
        if self.config.checkpoint_dir is None:
            ...
            self.config.checkpoint_dir = tempfile.mkdtemp(
                prefix="terradev_checkpoints_"
            )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Checkpoint manager initialized, {len(self.checkpoints)} existing checkpoints"
            )
            return True
    ...
    async def create_checkpoint(
        ...
        try:
            ...
            self.logger.info(
                f"Created KV cache checkpoint {checkpoint_id} ({size_bytes} bytes, {save_time_ms}ms)"
            )
    ...
    async def restore_checkpoint(
        self, checkpoint_id: str, request_id: str
    ) -> Optional[Any]:
        ...
        if checkpoint.state != CheckpointState.SAVED:
            self.logger.error(
                f"Checkpoint {checkpoint_id} not in saved state: {checkpoint.state}"
            )
            return None
        ...
        try:
            ...
            self.logger.info(
                f"Restored KV cache checkpoint {checkpoint_id} ({checkpoint.size_bytes} bytes, {load_time_ms}ms)"
            )
    ...
    async def handle_spot_termination(
        self, instance_id: str, provider: str, region: str
    ) -> bool:
        """Handle spot instance termination notice"""
        self.logger.warning(
            f"Spot termination notice received for {instance_id} on {provider}/{region}"
        )
        ...
        try:
                ...
                if checkpoint.state == CheckpointState.ACTIVE:
                    task = asyncio.create_task(
                        self._save_active_checkpoint(
                            checkpoint, instance_id, provider, region
                        )
                    )
            ...
            if save_tasks:
                try:
                    await asyncio.wait_for(
                        asyncio.gather(*save_tasks, return_exceptions=True), timeout=90
                    )
            ...
            if upload_success:
                self.logger.info(
                    f"Successfully saved {len(save_tasks)} KV caches before termination"
                )
                return True
            else:
    ...
    async def restore_on_new_instance(
        ...
        try:
            ...
            downloaded_checkpoints = await self._download_from_persistent_storage(
                instance_id
            )
            ...
            for checkpoint_id in downloaded_checkpoints:
                ...
                task = asyncio.create_task(
                    self.restore_checkpoint(checkpoint_id, request_id)
                )
            ...
            successful_restores = sum(
                1 for r in restore_results.values() if r["success"]
            )
            total_restores = len(restore_results)

            self.logger.info(
                f"Restored {successful_restores}/{total_restores} KV caches on new instance"
            )
            ...
            return {
                ...
                "success_rate": (
                    successful_restores / total_restores if total_restores > 0 else 0
                ),
            }
    ...
    async def _save_to_s3(self, checkpoint_id: str, data: bytes) -> str:
        ...
        s3_client.put_object(
            Bucket=bucket, Key=key, Body=data, ServerSideEncryption="AES256"
        )
    ...
    async def _save_active_checkpoint(
        self,
        checkpoint: KVCacheCheckpoint,
        instance_id: str,
        provider: str,
        ...
        try:
            ...
            nvme_path = (
                Path(self.config.nvme_path) / f"{checkpoint.checkpoint_id}.cache"
            )
    ...
    async def _upload_to_persistent_storage(self, instance_id: str) -> bool:
        ...
        try:
            ...
            for checkpoint_file in checkpoint_files:
                ...
                self.logger.debug(
                    f"Uploaded checkpoint {checkpoint_id} to {storage_path}"
                )

            self.logger.info(
                f"Uploaded {len(checkpoint_files)} checkpoints to persistent storage"
            )
            return True
    ...
    async def _cleanup_expired_checkpoints(self) -> None:
        ...
        for checkpoint_id in expired_checkpoints:
            ...
            self.active_checkpoints = {
                req_id: cp
                for req_id, cp in self.active_checkpoints.items()
                if cp.checkpoint_id != checkpoint_id
            }

        if expired_checkpoints:
            self.logger.info(
                f"Cleaned up {len(expired_checkpoints)} expired checkpoints"
            )
        ...
        if success:
            ...
            if total_saves == 1:
                ...
                self.metrics.avg_save_time_ms = (
                    self.metrics.avg_save_time_ms * (total_saves - 1) + save_time_ms
                ) / total_saves
        else:
            # Update success rate
            total_saves = self.metrics.total_checkpoints_created
        ...
        if success:
            ...
            if total_loads == 1:
                ...
                self.metrics.avg_load_time_ms = (
                    self.metrics.avg_load_time_ms * (total_loads - 1) + load_time_ms
                ) / total_loads
        else:
            # Update success rate
            total_loads = self.metrics.total_checkpoints_restored
