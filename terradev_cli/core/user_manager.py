...
class UserManager:
            ...
            try:
                ...
                for invite_data in data.get("invites", []):
                    invite = UserInvite(
                        ...
                        accepted_at=(
                            datetime.fromisoformat(invite_data["accepted_at"])
    ...
    def create_tenant(
        self, name: str, domain: str, settings: Optional[Dict[str, Any]] = None
    ) -> Tenant:
        """Create a new tenant"""
    ...
    def get_user_statistics(self, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        """Get user statistics"""
        users = (
            self.get_users_by_tenant(tenant_id)
            if tenant_id
            else list(self.users.values())
        )
    ...
    def bulk_import_users(
        self, users_data: List[Dict[str, Any]], tenant_id: str
    ) -> Dict[str, Any]:
