...
class PriceDiscoveryEngine:
    ...
    async def get_realtime_prices(
        ...
        for price_data in real_prices:
            # Check capacity
            capacity = await self._check_capacity(
                price_data["provider"], gpu_type, region
            )
    ...
    async def _get_real_price_data(
        ...
        try:
            ...
            price_series = get_price_series(
                gpu_type=gpu_type, hours=24  # Last 24 hours
            )
    ...
    async def _check_capacity(
        self, provider: str, gpu_type: str, region: Optional[str]
    ) -> str:
    ...
    async def _calculate_confidence_from_real_data(self, price_data: Dict) -> float:
        ...
        try:
            ...
            age_factor = max(
                0.1, 1.0 - (hours_old / 24)
            )  # 50% confidence after 12 hours
        ...
        try:
            ...
            for tick in price_series:
                ...
                by_provider[provider].append(
                    {"price": tick["price_hr"], "timestamp": tick["ts"]}
                )
    ...
    def is_data_stale(
        self, gpu_type: str, provider: str, max_age_hours: int = 6
    ) -> bool:
...
class BudgetOptimizationEngine:
    ...
    async def optimize_for_budget(
        ...
        price_infos = await self.price_engine.get_realtime_prices(
            requirements["gpu_type"], requirements.get("region")
        )
...
class CostPredictorML:
    ...
    async def predict(
        self, requirements: Dict[str, Any], price_info: PriceInfo
    ) -> float:
