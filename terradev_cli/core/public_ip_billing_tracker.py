...
class PublicIPBillingTracker:
    ...
    async def update_ip_status(
        self, ip_address: str, status: IPStatus, last_active: Optional[datetime] = None
    ) -> Dict[str, Any]:
        ...
        for record in self.ip_records:
            if record.ip_address == ip_address:
                ...
                return {
                    ...
                    "idle_hours": (datetime.now() - record.last_active).total_seconds()
                    / 3600,
                    "recommendations": self._get_ip_recommendations(record),
                }
    ...
    async def analyze_ip_costs(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_records = [
            record for record in self.ip_records if record.created_at >= cutoff_date
        ]
        ...
        for record in recent_records:
            ...
            hours_active = min(
                (datetime.now() - record.created_at).total_seconds() / 3600, days * 24
            )
            ...
            if record.status == IPStatus.IDLE:
                idle_hours = (
                    datetime.now() - record.last_active
                ).total_seconds() / 3600
    ...
    async def detect_idle_ips(
        self, idle_threshold_hours: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        for record in self.ip_records:
            if record.status == IPStatus.ACTIVE:
                ...
                if idle_hours >= idle_threshold_hours:
                    monthly_savings = (
                        record.hourly_cost * 730
                    )  # Monthly savings if released
                    ...
                    idle_ips.append(
                        {
                            ...
                            "last_active": record.last_active.isoformat(),
                            "recommendation": self._get_idle_ip_recommendation(
                                record, idle_hours
                            ),
    ...
    def _get_idle_ip_recommendation(
        self, record: PublicIPRecord, idle_hours: float
    ) -> str:
        ...
        if idle_hours > 168:  # 1 week
            ...
            return (
                f"IP idle for {idle_hours:.0f} hours - monitor for continued inactivity"
            )

    def _calculate_optimization_savings(
        self, records: List[PublicIPRecord], days: int
    ) -> float:
    ...
    async def _generate_cost_recommendations(
        self, records: List[PublicIPRecord]
    ) -> List[str]:
        ...
        if idle_count > 0:
            recommendations.append(
                f"Release {idle_count} idle IPs to save on monthly costs"
            )
    ...
    async def get_provider_summary(self, provider: str) -> Dict[str, Any]:
        ...
        provider_records = [
            r for r in self.ip_records if r.provider == provider.lower()
        ]
        ...
        for status in IPStatus:
            status_breakdown[status.value] = sum(
                1 for r in provider_records if r.status == status
            )
        ...
        return {
            ...
            "cost_per_ip": (
                round(hourly_cost_total / len(provider_records), 4)
                if provider_records
                else 0
            ),
