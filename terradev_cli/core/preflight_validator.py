...
class PreflightReport:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "counts": counts,
            "failures": [
                {"name": c.name, "node": c.node, "message": c.message}
        ...
        if tp_size > gpu_count:
            results.append(
                CheckResult(
                    ...
                    details={"tensor_parallel_size": tp_size, "gpu_count": gpu_count},
                    node=node,
                )
            )
        ...
        results.append(
            CheckResult(
                ...
                details={
                    ...
                    "recommendation": "batched_dispatch",
                },
                node=node,
            )
        )
...
def _check_gpu_inventory(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    for line in stdout.splitlines():
        ...
        if gpu["temp_c"] > 85:
            results.append(
                CheckResult(
                    f"gpu_{gpu['index']}_temp",
                    CheckStatus.WARN,
                    f"GPU {gpu['index']} at {gpu['temp_c']}°C",
                    details=gpu,
                    node=node,
                )
            )
        ...
        if mem_pct > 10:
            results.append(
                CheckResult(
                    f"gpu_{gpu['index']}_mem",
                    CheckStatus.WARN,
                    f"GPU {gpu['index']} has {mem_pct:.0f}% memory in use",
                    details=gpu,
                    node=node,
                )
            )
    ...
    if expected_gpus and actual_count != expected_gpus:
        results.append(
            CheckResult(
                ...
                details={"expected": expected_gpus, "actual": actual_count},
                node=node,
            )
        )
    elif actual_count > 0:
        ...
        if expected_type.lower() not in gpus[0]["name"].lower():
            results.append(
                CheckResult(
                    "gpu_type",
                    CheckStatus.WARN,
                    f"Expected {expected_type}, found {gpus[0]['name']}",
                    node=node,
                )
            )
...
def _check_dcgm_diag(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    rc, stdout, stderr = _run_on(
        host,
        f"dcgmi diag --run {level} --fail-early -j 2>/dev/null || echo DCGM_NOT_FOUND",
        user,
        key,
        timeout=300,
    )
    ...
    if "DCGM_NOT_FOUND" not in stdout and rc == 0:
        ...
        try:
            ...
            passed = True
            details = {}
            for test in diag.get(
                "tests", diag.get("DCGM Diagnostic", {}).get("test_categories", [])
            ):
            ...
            return [
                CheckResult(
                    "dcgm_diag",
                    CheckStatus.PASS,
                    f"DCGM level-{level}: all tests passed",
                    node=node,
                )
            ]
    ...
    rc2, stdout2, stderr2 = _run_on(
        host,
        "which gpu_burn >/dev/null 2>&1 && gpu_burn 30 2>&1 || echo GPU_BURN_NOT_FOUND",
        user,
        key,
        timeout=60,
    )
    ...
    if "OK" in stdout2 and "FAIL" not in stdout2:
        return [
            CheckResult(
                "gpu_burn", CheckStatus.PASS, "gpu-burn 30s: all GPUs OK", node=node
            )
        ]
    return [
        CheckResult(
            "gpu_burn", CheckStatus.FAIL, f"gpu-burn failed: {stdout2[:300]}", node=node
        )
    ]
...
def _check_nvlink_topo(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    rc, stdout, _ = _run_on(
        host,
        "which nvbandwidth >/dev/null 2>&1 && nvbandwidth -t device_to_device_memcpy_write_ce 2>&1"
        " || echo NVB_NOT_FOUND",
        user,
        key,
        timeout=120,
    ...
    if "NVB_NOT_FOUND" not in stdout and rc == 0:
        ...
        if bw_values:
            ...
            status = (
                CheckStatus.PASS
                if min_bw > 300
                else (CheckStatus.WARN if min_bw > 200 else CheckStatus.FAIL)
            )
            results.append(
                CheckResult(
                    "nvbandwidth",
...
def _check_rdma_health(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    if down > 0:
        results.append(
            CheckResult(
                ...
                details={"active": active, "down": down},
                node=node,
            )
        )
    elif active > 0:
        ...
        results.append(
            CheckResult(
                ...
                details={"active": active, "rates_gbps": rates},
                node=node,
            )
        )
    ...
    if "SKIP" not in stdout:
        ...
        if errors:
            ...
            results.append(
                CheckResult(
                    "ib_errors", CheckStatus.PASS, "No IB link errors", node=node
                )
            )
    ...
    if "nvidia_peermem" in stdout:
        results.append(
            CheckResult(
                "gpudirect_rdma", CheckStatus.PASS, "nvidia_peermem loaded", node=node
            )
        )
    else:
        results.append(
...
def _check_storage_fio(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    if "FIO_NOT_FOUND" not in stdout and rc == 0:
        try:
            ...
            if jobs:
                write_bw_mbs = (
                    jobs[0].get("write", {}).get("bw_mean", 0) / 1024
                )  # KB/s → MB/s
                write_iops = jobs[0].get("write", {}).get("iops_mean", 0)
                status = (
                    CheckStatus.PASS
                    if write_bw_mbs > 500
                    else (CheckStatus.WARN if write_bw_mbs > 100 else CheckStatus.FAIL)
                )
                return [
                    CheckResult(
                        "storage_fio",
    ...
    if match:
        ...
        status = (
            CheckStatus.PASS
            if bw_mbs > 500
            else (CheckStatus.WARN if bw_mbs > 100 else CheckStatus.FAIL)
        )
        return [
            CheckResult(
                "storage_dd",
    ...
    return [
        CheckResult(
            "storage_io",
            CheckStatus.WARN,
            "I/O test ran but throughput not parsed",
            node=node,
        )
    ]
...
def _check_nccl_intra(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    if isinstance(gpu_inv, list) and any(
        getattr(r, "status", None) == CheckStatus.FAIL
        for r in gpu_inv
        ...
        return [
            CheckResult(
                "nccl_intra",
                CheckStatus.SKIP,
                "Skipped — GPU inventory check failed",
                node=node,
            )
        ]
    ...
    if "NCCL_NOT_FOUND" in stdout:
        return [
            CheckResult(
                "nccl_intra",
                CheckStatus.SKIP,
                "nccl-tests not installed — github.com/NVIDIA/nccl-tests",
                node=node,
            )
        ]
    if rc != 0:
        return [
            CheckResult(
                "nccl_intra",
                CheckStatus.FAIL,
                f"NCCL all-reduce failed: {(stdout + stderr)[:300]}",
                node=node,
            )
        ]
    ...
    if busbw_values:
        ...
        status = (
            CheckStatus.PASS
            if min_bw > 300
            else (CheckStatus.WARN if min_bw > 100 else CheckStatus.FAIL)
        )
        return [
            CheckResult(
                "nccl_intra",
    ...
    return [
        CheckResult(
            "nccl_intra",
            CheckStatus.PASS,
            "NCCL all-reduce completed (bandwidth not parsed)",
            node=node,
        )
    ]
...
def _check_ncu_stall_profile(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    rc, stdout, _ = _run_on(
        host, "which ncu 2>/dev/null || echo NCU_NOT_FOUND", user, key, timeout=5
    )
    if "NCU_NOT_FOUND" in stdout or rc != 0:
        return [
            CheckResult(
                ...
                details={
                    "taxonomy": {
                        k: v["label"] for k, v in _NCU_STALL_SIGNATURES.items()
                    }
                },
                node=node,
            )
    ...
    for line in stdout.splitlines():
        ...
        for sig_key in _NCU_STALL_SIGNATURES:
            if any(sig_key in p for p in parts):
                try:
                    stall_pcts[sig_key] = max(
                        stall_pcts.get(sig_key, 0.0), float(parts[-1])
                    )
                except (ValueError, IndexError):
                    pass
    ...
    status = (
        CheckStatus.PASS
        if dominant_pct < 30
        else CheckStatus.WARN if dominant_pct < 60 else CheckStatus.FAIL
    )
...
def _check_data_integrity(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    if not data_path:
        return [
            CheckResult(
                "data_integrity", CheckStatus.SKIP, "No data path configured", node=node
            )
        ]
    ...
    if os.path.isdir(data_path):
        ...
        results.append(
            CheckResult(
                ...
                details={"files": len(data_files), "bytes": total},
                node=node,
            )
        )
    else:
        ...
        if size == 0:
            return [
                CheckResult(
                    "data_empty",
                    CheckStatus.FAIL,
                    f"File empty: {data_path}",
                    node=node,
                )
            ]
        results.append(
            CheckResult(
                "data_exists", CheckStatus.PASS, f"{size / (1 << 30):.2f} GB", node=node
            )
        )
    ...
    if manifest_path and os.path.exists(manifest_path):
        try:
            ...
            if shards:
                verify_dag = DAGExecutor(
                    max_workers=min(len(shards), 8), name="data_checksum"
                )
                ...
                bad = sum(
                    1 for v in vr.outputs.values() if v in ("mismatch", "missing")
                )
                if bad > 0:
                    results.append(
                        CheckResult(
...
def _check_nccl_pair(ctx: Dict[str, Any]) -> List[CheckResult]:
    ...
    if "PAIR_NCCL_FAILED" in output or rc != 0:
        return [
            CheckResult(
                f"nccl_pair_{pair_label}",
                CheckStatus.FAIL,
                f"Cross-node NCCL failed: {pair_label}",
                details={"output": output[:300]},
                node=pair_label,
            )
        ]
...
def _build_node_dag() -> DAGExecutor:
    ...
    dag.add_node(
        "nccl_intra", _check_nccl_intra, depends_on={"gpu_inventory", "rdma_health"}
    )
    dag.add_node(
        "ncu_stall_profile", _check_ncu_stall_profile, depends_on={"gpu_inventory"}
    )
    return dag
...
class PreflightValidator:
    ...
    def run_all(self) -> PreflightReport:
        ...
        for dag_result in node_dag.batch_apply(contexts, fail_fast=False):
            ...
            for name, error in dag_result.errors.items():
                report.checks.append(
                    CheckResult(name, CheckStatus.FAIL, f"Crashed: {error}")
                )

        # Phase 2: Cross-node pairwise NCCL (if >1 node and no failures)
        real_nodes = [
            n for n in self.nodes if n and n not in ("localhost", "127.0.0.1")
        ]
        if len(real_nodes) >= 2 and not self.skip_cross_node and report.passed:
            pairs = [
                (real_nodes[i], real_nodes[i + 1])
        ...
        logger.info(
            f"Preflight: {len(report.checks)} checks, "
