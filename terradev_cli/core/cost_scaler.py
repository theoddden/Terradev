...
class CostConfig:
    ...
    peak_hours: List[int] = field(
        default_factory=lambda: [9, 10, 11, 14, 15, 16, 17, 18]
    )
...
class CostScaler:
    ...
    def get_current_hourly_cost(self) -> float:
        ...
        recent_penalties = sum(
            cost
            for model_id, cost in self.model_load_costs.items()
    ...
    def should_load_model(
        self, model_id: str, estimated_memory_gb: float
    ) -> Tuple[bool, str]:
        """
        ...
        if new_total_cost > self.config.hourly_budget_usd:
            if self.config.strategy == CostStrategy.BUDGET_CONSTRAINED:
                return (
                    False,
                    f"Would exceed budget: ${new_total_cost:.2f} > ${self.config.hourly_budget_usd:.2f}/hour",
                )

        # Strategy-specific decisions
        if self.config.strategy == CostStrategy.MINIMIZE_COST:
            return self._should_load_minimize_cost(
                model_id, estimated_memory_gb, new_total_cost
            )
        elif self.config.strategy == CostStrategy.BALANCE_COST_LATENCY:
            return self._should_load_balance(
                model_id, estimated_memory_gb, new_total_cost
            )
        elif self.config.strategy == CostStrategy.LATENCY_CRITICAL:
            return self._should_load_latency_critical(
                model_id, estimated_memory_gb, new_total_cost
            )
        elif self.config.strategy == CostStrategy.BUDGET_CONSTRAINED:
            return self._should_load_budget_constrained(
                model_id, estimated_memory_gb, new_total_cost
            )

        return True, "Default: load allowed"

    def _should_load_minimize_cost(
        self, model_id: str, memory_gb: float, new_cost: float
    ) -> Tuple[bool, str]:
        ...
        if new_cost > self.config.cost_threshold_for_warming:
            return (
                False,
                f"Cost too high: ${new_cost:.2f} > ${self.config.cost_threshold_for_warming:.2f}",
            )
    ...
    def _should_load_balance(
        ...
        if new_cost > adjusted_threshold:
            return (
                False,
                f"Cost exceeds adjusted threshold: ${new_cost:.2f} > ${adjusted_threshold:.2f}",
            )
    ...
    def _should_load_latency_critical(
        self, model_id: str, memory_gb: float, new_cost: float
    ) -> Tuple[bool, str]:
        ...
        if new_cost > self.config.hourly_budget_usd * 2:
            return (
                False,
                f"Severely over budget: ${new_cost:.2f} > ${self.config.hourly_budget_usd * 2:.2f}",
            )

        return True, "Latency critical: prioritize performance"

    def _should_load_budget_constrained(
        self, model_id: str, memory_gb: float, new_cost: float
    ) -> Tuple[bool, str]:
        ...
        if new_cost > self.config.hourly_budget_usd:
            return (
                False,
                f"Exceeds budget: ${new_cost:.2f} > ${self.config.hourly_budget_usd:.2f}",
            )
        ...
        if self.metrics.budget_utilization_percent > 80:
            return (
                False,
                f"Budget utilization too high: {self.metrics.budget_utilization_percent:.1f}%",
            )
    ...
    def register_model_load(self, model_id: str, memory_gb: float, load_time_s: float):
        ...
        logger.info(
            f"Model {model_id} loaded: {memory_gb:.1f}GB, ${load_cost:.3f} penalty"
        )
    ...
    def predict_hourly_cost(self, hours_ahead: int = 1) -> float:
        ...
        if not historical_costs:
            ...
            current_cost = self.get_current_hourly_cost()
            return current_cost * (
                1.5 if target_hour in self.config.peak_hours else 1.0
            )
    ...
    async def _record_hourly_cost(self):
        ...
        logger.debug(
            f"Cost recorded: ${current_cost:.3f}/hour, "
