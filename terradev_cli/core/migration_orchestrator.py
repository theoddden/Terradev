...
class MigrationOrchestrator:
    ...
    def plan_migration(
        ...
        if not source_workload:
            raise ValueError(
                f"Workload not found: provider={source_provider}, instance={instance_id}, workload={workload_id}"
            )

        # Get target pricing (mock for lightweight version)
        target_pricing = self._get_target_pricing(
            target_provider, source_workload.gpu_type
        )

        # Calculate transfer costs
        transfer_cost = self._calculate_transfer_cost(
            source_workload.provider,
            source_workload.region,
            target_provider,
            source_workload.region,
        ...
        compatibility = self._check_gpu_compatibility(
            source_workload.gpu_type, target_gpu
        )

        # Build migration steps
        steps = self._build_migration_steps(
            source_workload, target_provider, target_gpu
        )
        ...
        warnings = self._generate_warnings(
            source_workload, target_provider, compatibility
        )
        ...
        return MigrationPlan(
            ...
            confidence_score=self._calculate_confidence(
                source_workload, target_provider
            ),
            warnings=warnings,
        )

    def _find_workload(
        self, provider: str, instance_id: Optional[str], workload_id: Optional[str]
    ...
    def _calculate_transfer_cost(
        ...
        try:
            ...
            direct_cost = estimate_egress_cost(
                src_provider, src_region, dst_provider, dst_region, size_gb
            )
    ...
    def _check_gpu_compatibility(
        self, source_gpu: str, target_gpu: str
    ) -> Dict[str, Any]:
        """Check GPU compatibility and performance delta"""
        performance_delta = self.gpu_compatibility.get(source_gpu, {}).get(
            target_gpu, 1.0
        )
    ...
    def _build_migration_steps(
        self, workload: WorkloadState, target_provider: str, target_gpu: str
    ) -> List[str]:
    ...
    def _calculate_total_costs(
        self, workload: WorkloadState, target_hourly: float, transfer_cost: float
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_monthly_savings": round(
                (current_hourly - target_hourly) * 24 * 30, 2
            ),
        }
    ...
    def _generate_warnings(
        ...
        if compatibility["performance_delta"] < 0.8:
            warnings.append(
                f"Performance degradation: {compatibility['performance_change']}"
            )
        ...
        if workload.provider == target_provider:
            warnings.append(
                "Same-provider migration - consider instance upgrade instead"
            )

        return warnings

    def _calculate_confidence(
        self, workload: WorkloadState, target_provider: str
    ) -> float:
