...
class CUDAGraphIntegrator:
    ...
    def analyze_model_endpoint(
        self, model_id: str, endpoint_id: str, gpu_index: Optional[int] = None
        ...
        numa_scorecard = self.numa_scorer.score_endpoint(
            endpoint_id=endpoint_id, gpu_index=gpu_index, model_type=model_type
        )
    ...
    def _detect_model_type(self, model_id: str) -> str:
        ...
        if any(keyword in model_id_lower for keyword in ["moe", "mixture", "expert"]):
            ...
            for keyword in ["llama", "bert", "gpt", "transformer", "t5"]
        ):
            return "transformer"
        elif any(
            keyword in model_id_lower for keyword in ["resnet", "conv", "cnn", "vision"]
        ):
            return "cnn"
        else:
            return "unknown"
    ...
    def _update_warm_pool_priority(self, recommendation: CUDAGraphRecommendation):
        ...
        current_priority = self.warm_pool.model_priorities.get(
            recommendation.model_id, 0
        )
        ...
        logger.info(
            f"Updated priority for CUDA Graph model {recommendation.model_id}: {current_priority} -> {new_priority}"
        )
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        compatible = sum(
            1 for r in self._recommendations_cache.values() if r.use_cuda_graphs
        )
        high_potential = sum(
            1
            for r in self._recommendations_cache.values()
            if r.optimization_score > 0.8
        )
        numa_optimal = sum(
            1
            for r in self._recommendations_cache.values()
            if r.numa_alignment in ("PIX", "PXB")
        )
        avg_score = (
            sum(r.optimization_score for r in self._recommendations_cache.values())
        ...
        return {
            ...
            "optimization_potential": (
                f"{(high_potential / total_recommendations * 100):.1f}%"
                if total_recommendations > 0
                else "0%"
            ),
        }

    def get_recommendations_for_model(
        self, model_id: str
    ) -> List[CUDAGraphRecommendation]:
    ...
    def should_prefer_endpoint(
        self,
        model_id: str,
        endpoint1_id: str,
        endpoint2_id: str,
        gpu1_index: Optional[int] = None,
        gpu2_index: Optional[int] = None,
    ) -> Optional[str]:
        """
        ...
        if rec1.numa_alignment in ("PIX", "PXB") and rec2.numa_alignment not in (
            "PIX",
            "PXB",
        ):
            return endpoint1_id
        elif rec2.numa_alignment in ("PIX", "PXB") and rec1.numa_alignment not in (
            "PIX",
            "PXB",
        ):
            return endpoint2_id
...
def get_cuda_graph_integrator(
    numa_scorer: NUMAEndpointScorer, warm_pool: WarmPoolManager
) -> CUDAGraphIntegrator:
...
def analyze_endpoint_for_graphs(
    model_id: str, endpoint_id: str, gpu_index: Optional[int] = None
) -> CUDAGraphRecommendation:
    """
