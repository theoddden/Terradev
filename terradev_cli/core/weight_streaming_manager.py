...
class WeightStreamingManager:
    ...
    def __init__(self, config: StreamingConfig):
        ...
        self.metrics = StreamingMetrics(
            model_id=config.model_id, start_time=datetime.now()
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info(
                f"Initializing weight streaming for {self.config.model_id}"
            )
    ...
    async def start_streaming(
        ...
        try:
            ...
            if self.state == StreamingState.STREAMING:
                ...
                self.logger.info(
                    f"Weight streaming completed in {self.metrics.total_time_ms}ms"
                )
    ...
    async def _generate_layer_chunks(self) -> None:
        ...
        for i in range(0, self.config.total_layers, self.config.chunk_size_layers):
            ...
            ) * 1_000_000_000  # ~1GB per layer estimate
        ...
        while self.state == StreamingState.STREAMING:
            try:
                ...
                if success:
                    ...
                    self.metrics.total_layers_loaded += (
                        chunk.layer_end - chunk.layer_start
                    )

                    # Check if we can generate first token
                    if (
                        not self.metrics.first_token_time
                        and self._can_generate_first_token()
                    ):
            ...
            async with self.session.get(chunk.download_url) as response:
                ...
                if self.config.verify_checksums:
                    if not await self._verify_checksum(chunk):
                        self.logger.error(
                            f"Checksum verification failed for {chunk.chunk_id}"
                        )
                        return False
    ...
    def _get_next_compute_chunk(self) -> Optional[LayerChunk]:
        ...
        for chunk in self.chunks:
            if (
                chunk.downloaded
                and not chunk.loaded
                and chunk.chunk_id not in self.active_computes
            ):
        ...
        if self.metrics.start_time:
            ...
            if elapsed > 0:
                ...
                self.metrics.download_speed_mbps = (
                    self.metrics.total_bytes_downloaded / (1024 * 1024)
                ) / elapsed

                # Calculate compute speed
                self.metrics.compute_speed_layers_per_sec = (
                    self.metrics.total_layers_loaded / elapsed
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress": {
                "download": sum(1 for c in self.chunks if c.downloaded)
                / len(self.chunks)
                * 100,
                "load": sum(1 for c in self.chunks if c.loaded)
                / len(self.chunks)
                * 100,
            },
            "metrics": {
