...
class MIGManager:
    ...
    async def check_mig_support(
        self, gpu_type: str, instance_type: str
    ) -> Dict[str, Any]:
        ...
        provider_compatible = await self._check_provider_compatibility(
    ...
    async def _check_provider_compatibility(
        self, gpu_type: str, instance_type: str
    ) -> Dict[str, Any]:
    ...
    async def _check_aws_mig_compatibility(
        self, gpu_type: str, instance_type: str
    ) -> Dict[str, Any]:
    ...
    async def _check_gcp_mig_compatibility(
        self, gpu_type: str, instance_type: str
    ) -> Dict[str, Any]:
    ...
    async def _check_azure_mig_compatibility(
        self, gpu_type: str, instance_type: str
    ) -> Dict[str, Any]:
        ...
        mig_azure_series = {
            ...
            ],  # A30 not directly available, using A100 series as proxy
        }

        compatible_instances = [
            inst
            for inst in mig_azure_series.get(gpu_type, [])
            if instance_type.startswith(inst.split("_")[0])
        ]
    ...
    async def enable_mig(
        self, gpu_type: str, instance_id: str, provider_executor
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await provider_executor.execute_command(
                instance_id, enable_command, async_exec=False
            )

            if result.get("exit_code", 1) == 0:
                # Verify MIG is enabled
                mig_status = await self._check_mig_status(
                    instance_id, provider_executor
                )
                return {
                    "success": True,
    ...
    async def _check_mig_status(
        self, instance_id: str, provider_executor
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await provider_executor.execute_command(
                instance_id, mig_mode_cmd, async_exec=False
            )
            ...
            if mig_mode == "Enabled":
                ...
                list_result = await provider_executor.execute_command(
                    instance_id, mig_list_cmd, async_exec=False
                )
                mig_devices = [
                    line.strip()
                    for line in list_result.get("stdout", "").split("\n")
                    if line.strip()
                ]
    ...
    async def create_mig_instances(
        self, gpu_type: str, instance_id: str, profiles: List[str], provider_executor
    ) -> Dict[str, Any]:
        ...
        try:
            for i, profile in enumerate(profiles):
                ...
                result = await provider_executor.execute_command(
                    instance_id, create_cmd, async_exec=False
                )
    ...
    async def get_mig_cost_analysis(
        self, gpu_type: str, instance_type: str, hourly_cost: float
    ) -> Dict[str, Any]:
        ...
        for profile_name, profile_config in gpu_info["profiles"].items():
            ...
            for utilization in utilization_scenarios:
                ...
                mig_instances_needed = max(
                    1, int(utilization * profile_config["instances"])
                )
                ...
                savings_percent = (
                    (savings / full_gpu_cost * 100) if full_gpu_cost > 0 else 0
                )
    ...
    async def recommend_mig_configuration(
        self, gpu_type: str, workload_memory_gb: float, compute_requirement: float
    ) -> Dict[str, Any]:
                ...
                recommendations.append(
                    {
                        ...
                        "waste_memory_gb": round(
                            profile_config["memory_gb"] - workload_memory_gb, 2
                        ),
                        "waste_compute": round(
                            profile_config["compute"] - compute_requirement, 2
                        ),
                    }
                )
