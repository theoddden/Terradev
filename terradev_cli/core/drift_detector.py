...
class DriftDetector:
    ...
    async def detect_drift(
        self, job: str, version: Optional[str] = None
    ) -> DriftReport:
        ...
        for manifest_node in manifest.nodes:
            actual_node = next(
                (n for n in actual_nodes if n["pod_id"] == manifest_node.pod_id), None
            )
    ...
    async def fix_drift(
        ...
        return {
            "status": (
                "fixed"
                if not final_report.drifted_nodes and not final_report.missing_nodes
                else "partial"
            ),
    ...
    async def _get_actual_state(
        self, manifest_nodes: List[ManifestNode]
    ) -> List[Dict[str, Any]]:
    ...
    async def _query_provider(
        self, provider_name: str, nodes: List[ManifestNode]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for instance in instances:
                if any(
                    node.instance_id == instance.get("instance_id") for node in nodes
                ):
                    our_nodes.append(
                        {
    ...
    def _has_drift(
        self, manifest_node: ManifestNode, actual_node: Dict[str, Any]
    ) -> bool:
        """Check if node has drifted from manifest"""
        return (
            manifest_node.status != actual_node.get("status")
            or manifest_node.gpus != actual_node.get("gpus")
            or manifest_node.gpu_type != actual_node.get("gpu_type")
            or manifest_node.region != actual_node.get("region")
        )
    ...
    async def _recreate_node(
        self, node: ManifestNode, manifest: Manifest
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await provider.provision_instance(
                instance_type=instance_type, region=node.region, gpu_type=node.gpu_type
            )
