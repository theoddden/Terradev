...
class AsyncConfigManager:
        ...
        async with lock:
            try:
                if not file_path.exists():
                    logger.debug(
                        f"Config file not found, returning empty dict: {filename}"
                    )
                    return {}
    ...
    async def save_json(
        self, filename: str, data: Dict[str, Any], create_backup: bool = True
    ) -> bool:
        """
    ...
    async def update_json(
        self, filename: str, updates: Dict[str, Any], merge: bool = True
    ) -> bool:
        """
    ...
    async def cleanup_cache(self):
        ...
        expired_files = [
            filename
            for filename, timestamp in self._cache_timestamps.items()
