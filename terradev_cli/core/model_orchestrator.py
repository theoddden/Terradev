...
class ModelInstance:
    ...
    def __lt__(self, other):
        """Priority queue comparison - higher priority = lower value"""
...
class ModelOrchestrator:
    ...
    def __init__(
        ...
        # Model management with priority queues
        self.models: Dict[str, ModelInstance] = {}
        self.loading_queue: List[Tuple[int, str]] = []  # (priority, model_id) heap
        self.serving_queue: List[Tuple[int, str]] = []  # (priority, model_id) heap
        self.queue_lock = asyncio.Lock()

        # Memory tracking with efficient lookup
        self.gpu_memory_usage: Dict[int, float] = {}
        ...
        warm_config = WarmPoolConfig(
            max_warm_models=15,
            min_warm_models=(
                5 if scaling_policy == ScalingPolicy.LATENCY_OPTIMIZED else 3
            ),
    ...
    def register_model(
        ...
        moe_profile: Optional[MoEProfile] = None,
    ) -> ModelInstance:
    ...
    async def load_model(self, model_id: str, force: bool = False) -> bool:
        ...
        estimated_memory = instance.metrics.memory_gb or self._estimate_model_memory(
            instance
        )
        should_load, cost_reason = self.cost_scaler.should_load_model(
            model_id, estimated_memory
        )

        if not should_load and not force:
            logger.info(
                f"Cost optimization prevented loading {model_id}: {cost_reason}"
            )
            return False
        ...
        try:
            ...
            if success:
                ...
                self.cost_scaler.register_model_load(
                    model_id, instance.metrics.memory_gb, instance.metrics.load_time_s
                )

                # Mark as warm in warm pool manager
                self.warm_pool_manager.mark_model_warm(
                    model_id, instance.metrics.load_time_s
                )

                logger.info(
                    f"Model {model_id} loaded and warmed ({instance.metrics.memory_gb:.1f}GB)"
                )
                return True
            else:
    ...
    async def _make_room_for_model(self, target_instance: ModelInstance):
        ...
        candidates = [
            ...
            if instance.state == ModelState.WARM
            and instance.priority < target_instance.priority
        ]
        ...
        for model_id, instance in candidates:
            ...
            logger.info(
                f"Evicted {model_id} to make room for {target_instance.model_id}"
            )
    ...
    async def _billing_optimized_eviction(self):
        ...
        for model_id, instance in self.models.items():
            if (
                instance.state == ModelState.WARM
                and now - instance.last_accessed > idle_threshold
            ):
    ...
    async def _latency_optimized_warming(self):
        ...
        if len(warm_models) > self.WARM_POOL_TARGET_SIZE:
            for model_id, _ in warm_models[self.WARM_POOL_TARGET_SIZE :]:
                logger.info(
                    f"Latency optimization: evicting less popular model {model_id}"
                )
                await self.evict_model(model_id)
    ...
    async def _hybrid_policy(self):
        ...
        for model_id, instance in self.models.items():
            ...
            should_evict = (
                instance.metrics.requests_per_hour < high_traffic_threshold
                and now - instance.last_accessed > recent_threshold
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_utilization_percent": (self.used_memory_gb / self.total_memory_gb)
            * 100,
        }
