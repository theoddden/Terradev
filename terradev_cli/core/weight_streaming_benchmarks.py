...
class WeightStreamingBenchmarks:
    ...
    async def benchmark_vs_traditional_loading(self) -> Dict[str, Any]:
        ...
        model_configs = [
            {
                "name": "Small (7B)",
                "total_layers": 32,
                "expected_streaming_time": 60,
                "expected_traditional_time": 300,
            },
            {
                "name": "Medium (70B)",
                "total_layers": 80,
                "expected_streaming_time": 120,
                "expected_traditional_time": 1800,
            },
            {
                "name": "Large (671B)",
                "total_layers": 160,
                "expected_streaming_time": 180,
                "expected_traditional_time": 2700,
            },
        ]
        ...
        for config in model_configs:
            ...
            streaming_time = await self._simulate_weight_streaming(
                total_layers=config["total_layers"], enable_streaming=True
            )

            # Simulate traditional loading
            traditional_time = await self._simulate_weight_streaming(
                total_layers=config["total_layers"], enable_streaming=False
            )

            # Calculate improvement factor
            improvement_factor = (
                traditional_time / streaming_time if streaming_time > 0 else 0
            )

            # Check if meets expectations
            streaming_ok = (
                streaming_time <= config["expected_streaming_time"] * 1.2
            )  # 20% tolerance
    ...
    async def benchmark_different_model_sizes(self) -> Dict[str, Any]:
        ...
        for model in model_sizes:
            streaming_time = await self._simulate_weight_streaming(
                total_layers=model["layers"], enable_streaming=True
            )
    ...
    async def benchmark_network_conditions(self) -> Dict[str, Any]:
        ...
        for condition in network_conditions:
            ...
            time_ok = (
                streaming_time <= condition["expected_time_s"] * 1.5
            )  # 50% tolerance
    ...
    async def benchmark_storage_backends(self) -> Dict[str, Any]:
        ...
        local_result = next(
            (r for r in results if r["storage_backend"] == "local"), None
        )
        local_ok = local_result and local_result["streaming_time_s"] == min(
            r["streaming_time_s"] for r in results
        )
    ...
    async def _simulate_weight_streaming(
        ...
        if enable_streaming:
            ...
            parallel_multiplier = min(parallel_downloads, 8) * (
                1 + parallel_downloads * 0.15
            )
...
async def main():
    ...
    streaming_benchmark = next(
        (
            ...
            if "vs Traditional" in r["benchmark_name"]
        ),
        None,
    )
    ...
    parallel_benchmark = next(
        (
            r
            for r in results["benchmark_results"]
            if "Parallel Downloads" in r["benchmark_name"]
        ),
        None,
    )
    if parallel_benchmark and parallel_benchmark["status"] == "PASSED":
        print(
            f"  ✅ Parallel downloads improve performance: {parallel_benchmark.get('improvement_factor', 'N/A')}x"
        )
