...
class TriggerManager:
    ...
    def create_trigger(
        self, name: str, trigger_type: TriggerType, target_pipeline: str, **kwargs
    ) -> Trigger:
    ...
    def _handle_event(self, event: Event):
        ...
        for trigger in triggers:
            ...
            if (
                ...
                and trigger.event_type == event.type
            ):
                should_trigger = True
    ...
    def _evaluate_condition(self, condition: str, event: Event) -> bool:
        ...
        if (
            "drift" in condition.lower()
            and event.type == EventType.MODEL_DRIFT_DETECTED
        ):
...
class LineageService:
    ...
    def get_production_artifacts(
        ...
        artifacts = [
            a for a in self.artifacts.values() if a.environment == Environment.PROD
        ]
...
class EnvironmentManager:
    ...
    def approve_promotion(self, promotion_id: str, approved_by: str) -> bool:
        ...
        if promotion.artifact_id in self.lineage_service.artifacts:
            ...
            self.logger.info(
                f"Promotion completed: {promotion.from_env.value} -> {promotion.to_env.value}"
            )
