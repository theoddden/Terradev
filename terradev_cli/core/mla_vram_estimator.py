...
class MLA_VRAMEstimator:
    ...
    def estimate_vram(
        ...
        activation_cache_gb = self._calculate_activation_cache(
            arch, context_tokens, batch_size
        )
        ...
        if target_vram_gb:
            ...
            if baseline_vram > target_vram_gb:
                ...
                if "nvfp4" in supported_methods and selected != "nvfp4":
                    ...
                    selected = "mx-fp4"
                elif "awq" in supported_methods and selected not in (
                    "awq",
                    "nvfp4",
                    "mx-fp4",
                ):
    ...
    def _calculate_model_weights(self, arch: ModelArchitecture) -> float:
        ...
        if arch.is_moe and arch.num_experts > 0:
            ...
            shared_ratio = (
                0.3  # ~30% of weights are shared (embeddings, layernorm, etc.)
            )
            shared_weights_gb = (
                arch.total_params_b * shared_ratio * arch.bytes_per_param
            )
            expert_weights_gb = (
                arch.total_params_b * (1 - shared_ratio) * arch.bytes_per_param
            )
    ...
    def _calculate_kv_cache(
        self, arch: ModelArchitecture, context_tokens: int, batch_size: int
    ) -> float:
        ...
        if arch.attention_type == AttentionType.MULTI_HEAD_LATENT:
            ...
            self.logger.debug(
                f"MLA KV cache: {standard_kv_gb:.2f}GB → {compressed_kv_gb:.2f}GB "
    ...
    def _calculate_activation_cache(
        self, arch: ModelArchitecture, context_tokens: int, batch_size: int
    ) -> float:
    ...
    def compare_standard_vs_mla(
        ...
        actual_estimate = self.estimate_vram(
            model_id, context_tokens, batch_size, target_gpu_vram_gb
        )
        ...
        try:
            standard_estimate = self.estimate_vram(
                model_id, context_tokens, batch_size, target_gpu_vram_gb
            )
        finally:
        ...
        # Calculate savings
        kv_cache_savings_gb = (
            standard_estimate.kv_cache_gb - actual_estimate.kv_cache_gb
        )
        ...
        return {
            ...
            "savings": {
                ...
                "kv_cache_compression_ratio": round(
                    standard_estimate.kv_cache_gb / actual_estimate.kv_cache_gb, 2
                ),
                "total_savings_gb": round(total_savings_gb, 2),
                "gpu_savings": gpu_savings,
                "cost_savings_percent": round(
                    (total_savings_gb / standard_estimate.total_gb) * 100, 1
                ),
            },
            "recommendation": self._get_mla_recommendation(
                actual_estimate, standard_estimate
            ),
        }

    def _get_mla_recommendation(
        self, mla_estimate: VRAMBreakdown, standard_estimate: VRAMBreakdown
    ) -> str:
        """Get recommendation based on MLA vs standard comparison"""

        if mla_estimate.gpu_count < standard_estimate.gpu_count:
            return (
                f"MLA reduces GPU requirements from {standard_estimate.gpu_count} to "
