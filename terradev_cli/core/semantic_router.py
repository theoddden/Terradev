...
class NUMAEndpointScorer:
    ...
    def score_endpoint(
        ...
        return NUMAScorecard(
            ...
            numa_node=numa_node,
            pcie_locality="SYS",
            locality_score=1.5,  # neutral score instead of 3.0 (worst)
            has_rdma=False,
            nccl_optimal=False,
            xcd_count=0,
            gpu_arch="",
            has_intra_gpu_numa=False,
            xcd_locality=None,
            xcd_locality_score=0.0,
        )
...
class SemanticRouter:
    ...
    def get_nccl_env_for_decision(self, decision: RoutingDecision) -> Dict[str, str]:
        ...
        if card.pcie_locality == "PIX":
            ...
            _nccl_fd, _nccl_path = tempfile.mkstemp(prefix="nccl_topo_", suffix=".xml")
            os.close(_nccl_fd)
            env["NCCL_TOPO_DUMP_FILE"] = _nccl_path
