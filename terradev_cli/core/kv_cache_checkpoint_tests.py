...
from core.kv_cache_checkpoint_manager import (
...
class KVCacheCheckpointTests:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Checkpoint test suite completed: {passed}/{total} tests passed"
            )
            return summary
    ...
    async def test_checkpoint_creation(self) -> Dict[str, Any]:
        ...
        if checkpoint_id:
            ...
            metadata_valid = (
                checkpoint.model_id == "test-model"
                and checkpoint.request_id == "test-request-1"
                and checkpoint.context_length == 8192
                and checkpoint.state == CheckpointState.SAVED
            )
        else:
        ...
        return {
            ...
            "status": (
                "PASSED"
                if creation_success and checkpoint_exists and metadata_valid
                else "FAILED"
            ),
    ...
    async def test_checkpoint_restoration(self) -> Dict[str, Any]:
        ...
        # Restore checkpoint
        restored_kv_cache = await manager.restore_checkpoint(
            checkpoint_id, "test-request-2-restored"
        )

        # Validate restoration
        restoration_success = restored_kv_cache is not None
        data_integrity = (
            self._compare_kv_cache_data(original_kv_cache, restored_kv_cache)
    ...
    async def test_data_integrity(self) -> Dict[str, Any]:
        ...
        for i, case in enumerate(test_cases):
            ...
            # Restore checkpoint
            restored_data = await manager.restore_checkpoint(
                checkpoint_id, f"restored-{i}"
            )

            # Validate integrity
            integrity_ok = (
                self._compare_kv_cache_data(original_data, restored_data)
                if restored_data
                else False
            )
    ...
    async def test_compression(self) -> Dict[str, Any]:
        ...
        if compressed_id and uncompressed_id:
            ...
            size_reduction = (
                uncompressed_checkpoint.size_bytes / compressed_checkpoint.size_bytes
            )
            compression_effective = size_reduction >= 2.0  # At least 2x reduction

            # Test restoration
            compressed_restored = await manager_compressed.restore_checkpoint(
                compressed_id, "restored-compressed"
            )
            uncompressed_restored = await manager_uncompressed.restore_checkpoint(
                uncompressed_id, "restored-uncompressed"
            )

            restoration_success = (
                compressed_restored is not None and uncompressed_restored is not None
            )
            data_match = (
                self._compare_kv_cache_data(compressed_restored, uncompressed_restored)
                if restoration_success
                else False
            )
        else:
        ...
        return {
            ...
            "status": (
                "PASSED"
                if compression_effective and restoration_success and data_match
                else "FAILED"
            ),
    ...
    async def test_spot_termination_handling(self) -> Dict[str, Any]:
        ...
        termination_success = await manager.handle_spot_termination(
            instance_id="test-instance-1", provider="aws", region="us-east-1"
        )
        ...
        restore_results = await manager.restore_on_new_instance(
            instance_id="test-instance-2", provider="aws", region="us-east-1"
        )
        ...
        return {
            ...
            "status": (
                "PASSED"
                if termination_success and all_saved and restore_success
                else "FAILED"
            ),
    ...
    async def test_cleanup_expired_checkpoints(self) -> Dict[str, Any]:
        ...
        if checkpoint_ids[0] in manager.checkpoints:
            manager.checkpoints[checkpoint_ids[0]].expires_at = (
                datetime.now() - timedelta(hours=2)
            )
        if checkpoint_ids[1] in manager.checkpoints:
            manager.checkpoints[checkpoint_ids[1]].expires_at = (
    ...
    async def test_storage_backends(self) -> Dict[str, Any]:
        ...
        local_restored = await manager_local.restore_checkpoint(
            local_checkpoint_id, "local-restored"
        )
    ...
    async def test_performance_metrics(self) -> Dict[str, Any]:
        ...
        # Validate metrics
        metrics_valid = (
            metrics.total_checkpoints_created == 5
            and metrics.total_data_saved_gb > 0
            and metrics.avg_save_time_ms > 0
        )

        status_valid = (
            status["active_checkpoints"] == 5
            and status["total_checkpoints"] == 5
            and status["metrics"]["total_created"] == 5
        )
    ...
    def _create_mock_kv_cache(
        self, context_length: int, num_layers: int, num_heads: int, head_dim: int
    ) -> Dict[str, Any]:
        ...
        for layer in range(num_layers):
            ...
            for head in range(num_heads):
                ...
                head_keys = [
                    [0.1 * (i + j) for j in range(head_dim)]
                    for i in range(context_length)
                ]
                head_values = [
                    [0.2 * (i + j) for j in range(head_dim)]
                    for i in range(context_length)
                ]
    ...
    def _compare_kv_cache_data(self, original: Any, restored: Any) -> bool:
        ...
        try:
            ...
            if len(original.get("value_cache", [])) != len(
                restored.get("value_cache", [])
            ):
                return False
