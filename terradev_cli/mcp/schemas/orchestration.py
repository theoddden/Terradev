...
if Tool is not None:
    TOOLS = [
        ...
        Tool(name='cost_scaler_start', description='Start the budget-aware auto-scaling manager. 4 strategies: minimize_cost, balance_cost_latency, latency_critical, budget_constrained.', inputSchema={'type': 'object', 'properties': {'strategy': {'type': 'string', 'description': 'Cost strategy', 'enum': ['minimize_cost', 'balance_cost_latency', 'latency_critical', 'budget_constrained'], 'default': 'balance_cost_latency'}, 'budget': {'type': 'number', 'description': 'Hourly budget in USD', 'default': 15.0}, 'cost_per_gb': {'type': 'number', 'description': 'Cost per GB per hour', 'default': 0.1}}}),
        Tool(name='cost_scaler_status', description='Get cost scaler status with budget utilization, predictions, and optimization recommendations.', inputSchema={'type': 'object', 'properties': {}}),
    ]
