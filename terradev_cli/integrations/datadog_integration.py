...
def get_credential_prompts() -> List[Dict[str, str]]:
    return [
        ...
        {
            "key": "datadog_site",
            "prompt": "Datadog site (default: datadoghq.com)",
            "required": False,
            "hide": False,
        },
    ]
...
METRIC_CATALOG = {
    f"{PREFIX}.gpu.cost_per_hour": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Current GPU cost $/hr",
        "tags": ["provider", "gpu_type", "region", "instance_id", "spot"],
    },
    f"{PREFIX}.gpu.total_cost": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Accumulated total cost",
        "tags": ["provider", "instance_id"],
    },
    f"{PREFIX}.gpu.monthly_projected": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Projected monthly spend",
        "tags": ["provider"],
    },
    f"{PREFIX}.gpu.budget_utilization": {
        "type": "gauge",
        "unit": "percent",
        "desc": "Budget consumed %",
        "tags": [],
    },
    f"{PREFIX}.provisions.total": {
        "type": "count",
        "unit": "instance",
        "desc": "Total provisions",
        "tags": ["provider", "gpu_type", "region"],
    },
    f"{PREFIX}.provisions.active": {
        "type": "gauge",
        "unit": "instance",
        "desc": "Active instances",
        "tags": ["provider"],
    },
    f"{PREFIX}.provisions.duration_s": {
        "type": "gauge",
        "unit": "second",
    ...
    f"{PREFIX}.price.quote": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Latest quoted price",
        "tags": ["provider", "gpu_type", "region", "spot"],
    },
    f"{PREFIX}.price.delta_1h": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "1h price change",
        "tags": ["provider", "gpu_type"],
    },
    f"{PREFIX}.price.volatility": {
        "type": "gauge",
        "unit": "percent",
        "desc": "Annualized volatility",
        "tags": ["provider", "gpu_type"],
    },
    f"{PREFIX}.price.stability_score": {
        "type": "gauge",
        "unit": "score",
        "desc": "Stability score 0-100",
        "tags": ["provider", "gpu_type"],
    },
    f"{PREFIX}.provider.reliability": {
        "type": "gauge",
        "unit": "score",
        "desc": "Reliability score 0-100",
        "tags": ["provider"],
    },
    f"{PREFIX}.provider.latency_ms": {
        "type": "gauge",
        "unit": "millisecond",
        "desc": "Quote API latency",
        "tags": ["provider"],
    },
    f"{PREFIX}.provider.availability": {
        "type": "gauge",
        "unit": "percent",
        "desc": "GPU availability rate 24h",
        "tags": ["provider", "gpu_type"],
    },
    f"{PREFIX}.egress.bytes": {
        "type": "count",
        "unit": "byte",
        "desc": "Cross-cloud transfer",
        "tags": ["src_provider", "dst_provider"],
    },
    f"{PREFIX}.egress.cost": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Egress cost",
        "tags": ["src_provider", "dst_provider"],
    },
    f"{PREFIX}.training.gpu_util": {
        "type": "gauge",
        "unit": "percent",
        "desc": "Training GPU utilization",
        "tags": ["job_id", "gpu_type"],
    },
    f"{PREFIX}.training.cost_per_epoch": {
        "type": "gauge",
        "unit": "dollar",
        "desc": "Cost per epoch",
        "tags": ["job_id"],
...
def _build_series(metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for m in metrics:
        ...
        series.append(
            {
                "metric": m["metric"],
                "type": _DD_TYPE.get(m.get("type", "gauge"), 3),
...
def submit_metrics_sync(
    ...
    try:
        req = urllib.request.Request(
            url, data=payload, headers=_auth_headers(creds), method="POST"
        )
...
async def submit_metrics_async(
    creds: Dict[str, str], metrics: List[Dict[str, Any]]
) -> Dict[str, Any]:
    try:
        import aiohttp
...
async def send_event_async(
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(
                url,
                json=_build_event(title, text, alert_type, tags),
                headers=_auth_headers(creds),
                timeout=aiohttp.ClientTimeout(total=15),
            ) as r:
                if r.status < 300:
...
MONITOR_TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "spot_risk": {
        ...
        "options": {
            "thresholds": {"critical": 100, "warning": 60},
            "notify_no_data": False,
...
async def create_monitor(
    ...
    if template_name not in MONITOR_TEMPLATES:
        return {
            "success": False,
            "error": f"Unknown template: {template_name}. Available: {list(MONITOR_TEMPLATES.keys())}",
        }
...
def _create_monitor_sync(creds: Dict[str, str], monitor: Dict) -> Dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(
            url,
            data=json.dumps(monitor).encode(),
            headers=_auth_headers(creds),
            method="POST",
        )
        with urllib.request.urlopen(req, timeout=15) as resp:
...
async def create_all_monitors(creds: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        "monitors_created": ok,
        "monitors_failed": len(results) - ok,
        "details": results,
    }
...
def _gpu_cost_dashboard() -> Dict[str, Any]:
    ...
    def _qv(title, query, unit="", cond=None):
        w = {
            ...
            "requests": [
                {
                    "queries": [
                        {"data_source": "metrics", "name": "a", "query": query}
                    ],
                    "formulas": [{"formula": "a"}],
    ...
    return {
        ...
        "widgets": [
            {
                "definition": _qv(
                    "Hourly Spend", "sum:terradev.gpu.cost_per_hour{*}", "$/hr"
                ),
                "layout": {"x": 0, "y": 0, "width": 3, "height": 2},
            },
            {
                "definition": _qv(
                    "Monthly Projected", "avg:terradev.gpu.monthly_projected{*}", "$"
                ),
            ...
            },
            {
                "definition": _qv(
                    "Budget Used",
                    "avg:terradev.gpu.budget_utilization{*}",
                    "%",
                    budget_cond,
                ),
            ...
            {
                "definition": _ts(
                    "GPU Cost/hr by Provider",
                    "avg:terradev.gpu.cost_per_hour{*} by {provider}",
                    "bars",
                ),
            ...
            },
            {
                "definition": _ts(
                    "Quote Prices by GPU",
                    "avg:terradev.price.quote{*} by {gpu_type,provider}",
                ),
            ...
            },
            {
                "definition": _ts(
                    "Price Volatility",
                    "avg:terradev.price.volatility{*} by {provider,gpu_type}",
                ),
            ...
            },
            {
                "definition": _ts(
                    "Quote API Latency",
                    "avg:terradev.provider.latency_ms{*} by {provider}",
                ),
                "layout": {"x": 8, "y": 5, "width": 4, "height": 3},
            },
            {
                "definition": _ts(
                    "Training GPU Util", "avg:terradev.training.gpu_util{*} by {job_id}"
                ),
            ...
            },
            {
                "definition": _ts(
                    "Egress Cost",
                    "sum:terradev.egress.cost{*} by {src_provider,dst_provider}.as_count()",
                    "bars",
                ),
...
async def create_dashboard(
    creds: Dict[str, str], custom_title: Optional[str] = None
) -> Dict[str, Any]:
...
async def query_metrics(
    creds: Dict[str, str], query: str, from_seconds: int = 3600
) -> Dict[str, Any]:
    try:
        import aiohttp
    except ImportError:
        ...
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status < 300:
                    return {
                        "success": True,
                        "series_count": len(body.get("series", [])),
                        "data": body,
                    }
                return {"success": False, "error": f"HTTP {r.status}"}
    except Exception as e:
...
async def on_provision(
    ...
    metrics = [
        {
            "metric": f"{PREFIX}.gpu.cost_per_hour",
            "value": price_hr,
            "type": "gauge",
            "tags": {
                "provider": provider,
        ...
        {
            "metric": f"{PREFIX}.provisions.total",
            "value": 1,
            "type": "count",
    ...
    e_result = await send_event_async(
        creds,
        title=f"GPU Provisioned: {gpu_type} on {provider}",
...
async def on_terminate(
    ...
    metrics = [
        {
            "metric": f"{PREFIX}.gpu.total_cost",
            "value": total_cost,
            "type": "gauge",
            "tags": {"provider": provider, "instance_id": instance_id},
        },
        {
            "metric": f"{PREFIX}.provisions.duration_s",
    ...
    e_result = await send_event_async(
        creds,
        title=f"GPU Terminated: {instance_id}",
...
async def push_cost_snapshot(creds: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        if summary["days"] > 0 and summary["total_provision_cost"] > 0:
            ...
            metrics.append(
                {
                    "metric": f"{PREFIX}.gpu.monthly_projected",
                    "value": round(daily_avg * 30, 2),
                    "type": "gauge",
                    "tags": {},
                }
            )
        ...
        for prov, count in prov_counts.items():
            metrics.append(
                {
                    "metric": f"{PREFIX}.provisions.active",
                    "value": count,
                    "type": "gauge",
                    "tags": {"provider": prov},
                }
    ...
    try:
        ...
        for entry in ranking:
            ...
            metrics.append(
                {
                    "metric": f"{PREFIX}.provider.reliability",
                    "value": entry.get("overall_score", 0),
                    "type": "gauge",
                    "tags": {"provider": prov},
                }
            )
            if entry.get("avg_quote_latency_ms"):
                metrics.append(
                    {
                        "metric": f"{PREFIX}.provider.latency_ms",
                        "value": entry["avg_quote_latency_ms"],
                        "type": "gauge",
                        "tags": {"provider": prov},
                    }
                )
...
def generate_terraform_monitors() -> str:
    ...
    for key, tmpl in MONITOR_TEMPLATES.items():
        ...
        blocks.append(
            f"""resource "datadog_monitor" "terradev_{safe_key}" {{
  name    = "{tmpl['name']}"
  type    = "{tmpl['type']}"
  query   = "{tmpl['query']}"
  message = "{msg}"
