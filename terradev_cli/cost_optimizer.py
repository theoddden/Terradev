...
class InferXCostOptimizer:
    ...
    async def analyze_current_costs(
        self, cluster_config: Dict[str, Any], usage_metrics: Dict[str, Any]
    ) -> CostMetrics:
        ...
        storage_cost = (
            ...
            + cluster_config.get("snapshot_gb", 500)
            * 0.05  # $0.05/GB/month for snapshots
        ) / 730  # Convert to hourly

        # Calculate network costs
        network_cost = (
            usage_metrics.get("requests_per_hour", 100) * 0.000001
        ...
        models_per_gpu = usage_metrics.get("models_deployed", 10) / max(
            len(cluster_config.get("nodes", [])), 1
        )
    ...
    async def generate_optimization_recommendations(
        ...
        if current_metrics.gpu_utilization < rules["gpu_utilization_target"]:
            savings = (
                (rules["gpu_utilization_target"] - current_metrics.gpu_utilization)
                * 0.01
                * current_metrics.hourly_cost
                * 730
            )
        ...
        on_demand_nodes = [
            n for n in cluster_config.get("nodes", []) if not n.get("spot", True)
        ]
        if on_demand_nodes:
            spot_savings = 0.0
            for node in on_demand_nodes:
                ...
                cost_model = self.cost_models.get(gpu_type, self.cost_models["A100"])
                hourly_savings = (
                    cost_model["on_demand"] - cost_model["spot"]
        ...
        if target_tier == CostTier.ECONOMY:
            a100_nodes = [
                n
                for n in cluster_config.get("nodes", [])
                if n.get("gpu_type") == "A100"
            ]
            if a100_nodes:
                ...
                for node in a100_nodes:
                    ...
                    hourly_savings = (
                        cost_model_a100["spot"] - cost_model_a10g["spot"]
                    ) * node.get("gpu_count", 1)
        ...
        if current_metrics.models_per_gpu < rules["models_per_gpu_target"]:
            density_improvement = (
                rules["models_per_gpu_target"] - current_metrics.models_per_gpu
            )
            potential_savings = (
                density_improvement * 0.1 * current_metrics.hourly_cost * 730
            )
        ...
        if (
            current_metrics.storage_cost > current_metrics.hourly_cost * 0.1
        ):  # Storage > 10% of total cost
            storage_savings = (
                current_metrics.storage_cost * 0.5 * 730
            )  # 50% storage savings
        ...
        if current_metrics.cpu_utilization < rules["scale_down_threshold"]:
            scaling_savings = (
                current_metrics.hourly_cost * 0.3 * 730
            )  # 30% scaling savings
    ...
    async def simulate_optimization_scenario(
        ...
        scenario["optimized_monthly_cost"] = (
            current_metrics.monthly_cost - cumulative_savings
        )
        scenario["total_savings"] = cumulative_savings
        scenario["savings_percentage"] = (
            cumulative_savings / current_metrics.monthly_cost
        ) * 100
        ...
        medium_risk_count = sum(
            1 for rec in recommendations if rec.risk_level == "Medium"
        )
        ...
        scenario["risk_assessment"] = {
            ...
            "overall_risk": (
                ...
                else "Medium" if medium_risk_count > 3 else "Low"
            ),
        }

        # ROI analysis
        implementation_cost = (
            len(recommendations) * 500
        )  # $500 per recommendation implementation
        ...
        scenario["roi_analysis"] = {
            ...
            "annual_roi": (
                (monthly_savings * 12 / implementation_cost - 1) * 100
    ...
    async def generate_cost_report(
        ...
        current_metrics = await self.analyze_current_costs(
            cluster_config, usage_metrics
        )
        ...
        # Simulate optimization scenario
        scenario = await self.simulate_optimization_scenario(
        ...
        report = {
            ...
            "summary": {
                ...
                "payback_period_months": scenario["roi_analysis"][
                    "payback_period_months"
                ],
                "annual_roi": scenario["roi_analysis"]["annual_roi"],
            },
            "key_insights": self._generate_key_insights(
                current_metrics, recommendations, scenario
            ),
        }
    ...
    def _generate_key_insights(
        ...
        if scenario["savings_percentage"] > 50:
            ...
            insights.append(
                "Moderate cost optimization potential: >30% savings achievable"
            )
        else:
            insights.append(
                "Limited cost optimization potential: <30% savings achievable"
            )

        # Utilization insights
        if current_metrics.gpu_utilization < 50:
            insights.append(
                "Low GPU utilization indicates significant over-provisioning"
            )
        elif current_metrics.gpu_utilization > 90:
            insights.append(
                "High GPU utilization may impact performance and cold start times"
            )
        ...
        if current_metrics.models_per_gpu < 20:
            ...
            insights.append(
                "High model density indicates efficient resource utilization"
            )

        # Risk insights
        if scenario["risk_assessment"]["overall_risk"] == "High":
            insights.append(
                "High-risk optimizations require careful implementation and monitoring"
            )
