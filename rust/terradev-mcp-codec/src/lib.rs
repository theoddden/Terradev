...
impl MCPCodec {
    ...
    fn encode_tool_result(&self, id: String, content: PyObject, is_error: Option<bool>) -> PyResult<PyObject> {
        Python::with_gil(|py| {
    ...
    fn encode_batch(&self, results: PyObject) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            let results_vec: Vec<(String, Vec<serde_json::Value>, bool)> = if let Ok(list) = results.extract::<Vec<PyObject>>(py) {
                list.iter().map(|obj| {
            ...
            let mcp_results: Vec<MCPToolResult> = results_vec.into_iter()
