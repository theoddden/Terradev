...
class EvaluationOrchestrator:
    ...
    def evaluate_model(self, config: EvaluationConfig) -> EvaluationResult:
        ...
        if not config.model_path or not config.dataset_path:
            raise ValueError(
                "Model path and dataset path required for model evaluation"
            )
    ...
    def compare_models(
        ...
        config_a = EvaluationConfig(
            model_path=model_a_path, dataset_path=dataset_path, metrics=metrics
        )

        config_b = EvaluationConfig(
            model_path=model_b_path, dataset_path=dataset_path, metrics=metrics
        )
    ...
    def _calculate_accuracy(
        self, model_data: Any, dataset: Any, config: EvaluationConfig
    ) -> float:
    ...
    def _calculate_cost_per_token(
        self, endpoint_url: str, config: EvaluationConfig
    ) -> float:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        endpoint_url.replace("/v1/chat/completions", "/health"),
    ...
    def _compare_with_baseline(
        self, metrics: Dict[str, float], baseline_path: str, evaluation_id: str
    ) -> Dict[str, Any]:
            ...
            if baseline_file.exists():
                ...
                for metric, current_value in metrics.items():
                    ...
                    if baseline_value is not None:
                        ...
                        if improvement:
                            comparison["improvements"][metric] = comparison[
                                "differences"
                            ][metric]
                        elif diff != 0:
                            comparison["regressions"][metric] = comparison[
                                "differences"
                            ][metric]
