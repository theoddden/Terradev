...
class WarmPoolManager:
    ...
    def __init__(self, config: WarmPoolConfig, config_dir: Optional[Path] = None, cost_service: Optional[Any] = None):
        ...
        self.endpoint_numa_scores: Dict[str, Dict[str, Any]] = (
            {}
        ...
        async with self._lock:
            ...
            if was_warm:
                ...
                self.metrics.avg_warm_latency_ms = (
                    self.metrics.avg_warm_latency_ms * (self.metrics.cache_hits - 1)
                    + latency_ms
            ...
            self.model_traffic[model_id] = [
                timestamp
                for timestamp in self.model_traffic[model_id]
                if timestamp > cutoff
            ]
    ...
    async def _manage_warming(self):
        ...
        async with self._lock:
            ...
            available_slots = (
                self.config.max_warm_models
                - len(self.warm_models)
                - len(self.warming_models)
            )
    ...
    def get_predictive_warming_candidates(
    ...
    def get_status(self) -> Dict:
        ...
        cache_hit_rate = self.metrics.cache_hits / max(
            1, self.metrics.total_warm_requests
        )
    ...
    def get_model_details(self, model_id: str) -> Optional[Dict]:
        ...
        recent_traffic = [
            timestamp
            for timestamp in self.model_traffic.get(model_id, [])
            if now - timestamp < timedelta(hours=1)
        ]
    ...
    async def _analyze_cuda_graph_potential(self):
        ...
        for model_id in self.model_priorities.keys():
            ...
            if graph_score > 0.7:
                ...
                logger.info(
                    f"Model {model_id} identified as CUDA Graph compatible (score: {graph_score:.2f})"
                )
    ...
    async def _optimize_endpoint_selection(self):
        ...
        for model_id in self.cuda_graph_models:
            ...
            if best_endpoint and best_score > 0.7:
                ...
                if best_score > 0.9:
                    self.model_priorities[model_id] = max(
                        current_priority, 10
                    )  # Highest priority
                elif best_score > 0.8:
                    self.model_priorities[model_id] = max(
                ...
                logger.debug(
                    f"Prioritized endpoint {best_endpoint} for CUDA Graph model {model_id} (score: {best_score:.2f})"
                )
    ...
    def should_warm_with_cuda_graphs(self, model_id: str) -> bool:
        ...
        """
        return (
            model_id in self.cuda_graph_models
            and self.model_graph_scores.get(model_id, 0.0) > 0.7
        )
    ...
    def get_cuda_graph_optimization_tips(self, model_id: str) -> Dict[str, Any]:
        ...
        recommendations = {
            ...
            "memory_pool_size": (
                f"{int(graph_score * 4)}GB" if graph_score > 0.5 else "2GB"
            ),
