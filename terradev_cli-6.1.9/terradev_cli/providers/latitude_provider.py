...
class LatitudeProvider(BaseProvider):
    ...
    async def _get_bare_metal_quotes(
        self, gpu_type: str, region: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get bare metal server quotes"""
        try:
                            ...
                            quotes.append(
                                {
                                    ...
                                    "memory_gb": specs.get("memory", {}).get(
                                        "total", 0
                                    ),
                                    "gpu_count": gpu_info.get("count", 1),
                                    "gpu_memory_gb": gpu_info.get("vram_per_gpu", 0),
                                    "storage": specs.get("drives", [{}])[0].get(
                                        "size", "0"
                                    ),
                                    "network": specs.get("nics", [{}])[0].get(
                                        "type", "unknown"
                                    ),
                                    "spot": False,
                                    ...
                                    "ssh_access": True,
                                    "ipmi_access": True,
                                    "instant_deployment": region_info.get(
                                        "deploys_instantly", []
                                    ),
                                    "stock_level": region_info.get(
                                        "stock_level", "unknown"
                                    ),
                                }
                            )
    ...
    async def _get_virtual_machine_quotes(
        self, gpu_type: str, region: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def provision_instance(
        ...
        if instance_type.startswith("latitude-bare-metal"):
            return await self._provision_bare_metal(
                instance_type, region, gpu_type, **kwargs
            )
        elif instance_type.startswith("latitude-vm"):
            return await self._provision_virtual_machine(
                instance_type, region, gpu_type, **kwargs
            )
        else:
    ...
    async def _provision_bare_metal(
        ...
        try:
            ...
            result = await self._make_request(
                "POST", f"{self.API_BASE}/servers", json=server_data
            )
        ...
        try:
            ...
            try:
                data = await self._make_request(
                    "GET", f"{self.API_BASE}/servers/{instance_id}"
                )
        ...
        try:
            ...
            try:
                await self._make_request(
                    "POST",
                    f"{self.API_BASE}/servers/{instance_id}/actions",
                    json={"type": "power_off"},
                )
                return {
                    "instance_id": instance_id,
    ...
    async def terminate_instance(self, instance_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                await self._make_request(
                    "DELETE", f"{self.API_BASE}/servers/{instance_id}"
                )
                return {
                    "instance_id": instance_id,
                    "action": "terminate",
