...
class OVHcloudProvider(BaseProvider):
    ...
    def __init__(self, credentials: Dict[str, str]):
        ...
        self.api_base = self.API_ENDPOINTS.get(
            endpoint_name, self.API_ENDPOINTS["ovh-eu"]
        )
    ...
    def _sign(self, method: str, url: str, body: str, timestamp: str) -> str:
        ...
        return "$1$" + hashlib.sha1(to_sign.encode("utf-8")).hexdigest()  # nosec B324 - SHA1 is required by OVH API specification for HMAC signatures
    ...
    async def _ovh_request(
        ...
        if (
            not self.application_key
            or not self.application_secret
    ...
    async def _get_live_flavors(
        ...
        for flavor in data if isinstance(data, list) else []:
            ...
            if "gpu" not in ftype and "gpu" not in fname:
                continue
            if (
                gpu_lower not in fname
                and gpu_lower not in flavor.get("planCodes", "").lower()
            ):
                continue
            quotes.append(
                {
                    ...
                    "memory_gb": (
                        flavor.get("ram", 0) / 1024
                        if flavor.get("ram", 0) > 1024
    ...
    async def provision_instance(
        ...
        if not self.project_id:
            raise Exception(
                "OVHcloud project_id not configured — run `terradev configure ovhcloud`"
            )
        ...
        if not image_id:
            ...
            images = await self._ovh_request(
                "GET",
                f"/cloud/project/{self.project_id}/image?osType=linux&region={region}",
            )
            for img in images if isinstance(images, list) else []:
                if "ubuntu" in img.get("name", "").lower() and "22.04" in img.get(
                    "name", ""
                ):
                    image_id = img.get("id", "")
                    break
        ...
        return {
            "instance_id": data.get(
                "id", f"ovh-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            ),
