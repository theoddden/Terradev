...
class TestSGLangOptimizer:
    ...
    def test_hardware_detection_h100(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="NVIDIA H100 80GB, 81920\n"
            )
    ...
    def test_hardware_detection_h20(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="NVIDIA H20 96GB, 98304\n"
            )
    ...
    def test_model_type_detection_llama(self):
        ...
        model_type, config = self.optimizer.detect_model_type(
    ...
    def test_workload_type_detection_user_description(self):
        ...
        workload = self.service.detect_workload_type(
            "llama", "Multi-turn conversational AI agent with tool usage"
        )
        assert workload == WorkloadType.AGENTIC_CHAT

        # Test batch detection
        workload = self.service.detect_workload_type(
            "llama", "Batch processing dataset evaluation"
        )
        assert workload == WorkloadType.BATCH_INFERENCE

        # Test low latency detection
        workload = self.service.detect_workload_type(
            "llama", "Real-time API with low TTFT requirements"
        )
...
class TestAgenticChatOptimizations:
    ...
    def setup_method(self):
        ...
        self.hardware = HardwareProfile(
            gpu_type="H100", memory_gb=80, bandwidth_gbps=2000, architecture="hopper"
        )
    ...
    def test_agentic_chat_performance_summary(self):
        ...
        assert (
            "LPM schedule policy for prefix sharing" in summary["optimizations_applied"]
        )
        assert (
            "RadixAttention enabled for cache hits" in summary["optimizations_applied"]
        )
...
class TestMoEOptimizations:
    ...
    def test_deepseek_moe_optimization(self):
        ...
        config = SGLangConfig(
            model_path="deepseek-ai/DeepSeek-V3", workload_type=WorkloadType.MOE_MODEL
        )
...
class TestConfigurationValidation:
    ...
    def test_latency_concurrency_warning(self):
        ...
        assert any(
            "High max-running-requests may impact latency" in w for w in warnings
        )
    ...
    def test_moe_memory_warning(self):
        ...
        assert any(
            "High redundant expert count may exceed memory" in w for w in warnings
        )
