...
class CLIUXBattleTest:
    ...
    def test_help_discovery(self):
        ...
        assert (
            "Terradev optimization commands" in result.stdout
    ...
    def test_analyze_command_ux(self):
        ...
        try:
            ...
            assert (
                "🔍" in output or "Analyzing" in output
            ...
            if result.returncode == 0:
                ...
                assert (
                    "Recommended Optimizations" in output
    ...
    def test_error_handling_ux(self):
        ...
        assert (
            "file" in output.lower() or "exist" in output.lower()
    ...
    def test_dashboard_command_ux(self):
        ...
        if result.returncode == 0:
            assert (
                "📊" in output or "Dashboard" in output
            ), "Should show dashboard indicator"
            assert (
                "Overall Metrics" in output or "Total" in output
            ), "Should show metrics"
            assert (
                "CUCo" in output or "optimization" in output.lower()
            ), "Should show optimization data"
    ...
    def test_config_command_ux(self):
        ...
        output = result.stdout + result.stderr
        assert (
            "⚙️" in output or "Configuration" in output
        ), "Should show config indicator"
        assert (
            "Auto Optimize" in output or "enabled" in output
    ...
    def test_progress_indicators(self):
        ...
        try:
            ...
            ), f"Should show multiple progress indicators, found: {found_indicators}"

            # Should show structured sections
            assert (
                "Results" in output or "Optimization" in output
            ), "Should show results section"

            return {
                "progress_indicators": len(found_indicators),
    ...
    def test_output_formatting(self):
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                assert (
                    "dashboard_timestamp" in json_data
    ...
    def test_edge_cases(self):
        ...
        assert result.returncode in [
            0,
            1,
        ], "Should handle long deployment ID gracefully"
    ...
    def test_command_completion(self):
        ...
        assert (
            len(found_commands) >= len(main_commands) - 2
        ), f"Should list most commands, found: {found_commands}"
        ...
        assert (
            len(found_config_commands) >= 2
        ), f"Should list config subcommands, found: {found_config_commands}"
