...
class TestObserveFunctional:
    @patch("terradev_cli.core.observe.observe_gateway_traffic")
    def test_observe_gateway(self, mock_observe, runner):
        mock_observe.return_value = {
            "trace_id": "t1",
            "active_destinations": ["wandb"],
            "start_time": "now",
        }
        result = runner.invoke(cli, ["observe", "gateway", "http://localhost:8000"])
        assert result.exit_code == 0
        assert "t1" in result.output

    @patch("terradev_cli.core.observe.observe_status")
    def test_observe_status(self, mock_status, runner):
        mock_status.return_value = {
            "status": "ok",
            "trace_id": "t1",
            "wandb": "ok",
        }
        result = runner.invoke(cli, ["observe", "status", "t1"])
        assert result.exit_code == 0
        assert "ok" in result.output.lower()


class TestScheduleFunctional:
    @patch("terradev_cli.core.schedule.schedule_spot_job")
    def test_schedule_job(self, mock_job, runner):
        mock_job.return_value = {"status": "success", "job_name": "j1"}
        result = runner.invoke(cli, ["schedule", "job", "train.py", "A100", "--job-name", "j1"])
        assert result.exit_code == 0

    @patch("terradev_cli.core.schedule.schedule_list")
    def test_schedule_list(self, mock_list, runner):
        mock_list.return_value = ["j1"]
        result = runner.invoke(cli, ["schedule", "list"])
        assert result.exit_code == 0

    @patch("terradev_cli.core.schedule.schedule_pricing_windows")
    def test_schedule_windows(self, mock_win, runner):
        mock_win.return_value = {"A100": [{"start": 0, "end": 3600}]}
        result = runner.invoke(cli, ["schedule", "windows", "--gpu-type", "A100"])
        assert result.exit_code == 0
