...
class IntegrationTestFramework:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_suite in test_suites:
            ...
            try:
                ...
                print(
                    f"✅ {suite_name}: {result['passed']}/{result['total']} tests passed"
                )
    ...
    async def _test_cli_help(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["terradev", "--help"], capture_output=True, text=True, timeout=30
            )
    ...
    async def _test_version_command(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["terradev", "--version"], capture_output=True, text=True, timeout=30
            )
    ...
    async def _generate_test_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            f"Summary: {total_passed}/{total_tests} tests passed ({report['summary']['success_rate']:.1f}%)"
        )
...
@click.option(
    "--performance/--no-performance", default=False, help="Enable performance testing"
)
