...
class IntegratedOptimizationTest:
    ...
    def test_cost_optimization_only(self):
        ...
        assert (
            "COST OPTIMIZATION OPPORTUNITIES" in output
        ), "Should show cost opportunities"
    ...
    def test_cuco_integration_attempt(self):
        ...
        assert (
            "CUCO optimization not available" in output
        ), "Should show CUCo unavailable"
    ...
    def test_optimization_help(self):
        ...
        assert (
            "Multi-dimensional optimization" in output
        ), "Should show multi-dimensional description"
    ...
    def generate_performance_table(self):
        ...
        # Print the table
        print(
            f"{'Component':<25} {'Status':<12} {'Implementation':<15} {'P95':<8} {'P10':<8} {'Integration':<12}"
        )
        print("-" * 85)
        ...
        for component, data in performance_data.items():
            status = data["Status"]
            impl = (
                data["Implementation"][:12] + ".."
                if len(data["Implementation"]) > 12
                else data["Implementation"]
            )
            p95 = (
                data["P95 Compliance"][:6] + ".."
                if len(data["P95 Compliance"]) > 6
                else data["P95 Compliance"]
            )
            p10 = (
                data["P10 Compliance"][:6] + ".."
                if len(data["P10 Compliance"]) > 6
                else data["P10 Compliance"]
            )
            integration = (
                data["Integration"][:10] + ".."
                if len(data["Integration"]) > 10
                else data["Integration"]
            )

            print(
                f"{component:<25} {status:<12} {impl:<15} {p95:<8} {p10:<8} {integration:<12}"
            )
    ...
    def generate_compliance_matrix(self):
        ...
        print(
            f"{'Workload':<20} {'Current':<15} {'P95 Ready':<10} {'P10 Ready':<10} {'Status'}"
        )
        print("-" * 70)
        ...
        for workload, data in compliance_matrix.items():
            ...
            status = (
                data["Our Status"][:20] + ".."
                if len(data["Our Status"]) > 20
                else data["Our Status"]
            )

            print(
                f"{workload:<20} {current:<15} {p95_ready:<10} {p10_ready:<10} {status}"
            )
    ...
    def test_context_aware_logic(self):
        ...
        def mock_calculate_priority(
            instance_name, tags, gpu_count=4, communication_intensity=0.6
        ):
            base_priority = 1.2  # Mock base performance gain
            context_multiplier = 1.0

            high_throughput_indicators = [
                "inference",
                "serving",
                "api",
                "real-time",
            ...
            if any(
                indicator in instance_name.lower()
                for indicator in high_throughput_indicators
            ):
                context_multiplier *= 1.5
            elif any(
                indicator in str(tags).lower()
                for indicator in high_throughput_indicators
            ):
            ...
            if any(
                indicator in instance_name.lower()
                for indicator in bandwidth_critical_indicators
            ):
        ...
        for case in test_cases:
            calculated_priority = mock_calculate_priority(
                case["instance_id"], case["tags"]
            )
