...
def test_throughput_optimization():
    ...
    ), f"Expected 16384, got {config.max_num_batched_tokens}"
...
def test_latency_optimization():
    ...
    ), f"Expected 4096, got {config.max_num_batched_tokens}"
...
def test_auto_optimization():
    ...
    config = VLLMConfig.create_auto_optimized(
        "meta-llama/Llama-2-7b-hf", high_qps_workload
    )

    # Should optimize for high throughput
    assert (
        config.max_num_batched_tokens >= 16384
    ), f"High QPS should use large batch size, got {config.max_num_batched_tokens}"
    assert (
    ...
    assert (
        config.cpu_cores >= 6
    ), f"High QPS should allocate more CPU cores, got {config.cpu_cores}"
    ...
    config = VLLMConfig.create_auto_optimized(
        "mistralai/Mistral-7B-v0.1", latency_workload
    )

    # Should optimize for low latency
    assert (
        config.max_num_batched_tokens <= 4096
    ), f"Latency-focused should use smaller batch size, got {config.max_num_batched_tokens}"
    assert (
        config.max_num_seqs <= 512
    ), f"Latency-focused should limit sequences, got {config.max_num_seqs}"
...
def test_workload_analysis():
    ...
    samples = [
        ...
        {
            "prompt": "A much longer prompt with detailed context and requirements",
            "response": "A comprehensive response with multiple paragraphs and detailed explanations",
            "timestamp": 2000,
            "user_id": "user2",
        },
        {
...
def main():
    ...
    try:
        ...
        print("\n📋 Next steps:")
        print(
            "   1. Run: terradev vllm auto-optimize -s sample_workload.json -m your-model"
        )
