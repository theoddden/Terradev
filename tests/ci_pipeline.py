...
class CIPipeline:
    ...
    async def setup_environment(self) -> bool:
        ...
        subprocess.run(
            ["./test_env/bin/pip", "install", "-r", "requirements.txt"], check=True
        )
    ...
    async def run_security_scan(self) -> Dict[str, Any]:
        ...
        return {
            "bandit": {
                ...
                "issues_found": len(
                    json.loads(Path("bandit_report.json").read_text())["results"]
                ),
            },
            "safety": {
                "exit_code": safety_result.returncode,
                "vulnerabilities_found": len(
                    json.loads(Path("safety_report.json").read_text())
                ),
            },
        }
    ...
    async def build_package(self) -> Dict[str, Any]:
        ...
        flake8_result = subprocess.run(
            ["./test_env/bin/flake8", "terradev_cli/"], capture_output=True, text=True
        )

        # Build package
        build_result = subprocess.run(
            ["./test_env/bin/python", "-m", "build"], capture_output=True, text=True
        )
        ...
        return {
            ...
            "build": {
                ...
                "dist_files": (
                    list(Path("dist").glob("*")) if Path("dist").exists() else []
                ),
            },
        }
    ...
    async def deploy_to_staging(self) -> Dict[str, Any]:
        ...
        return {"environment": "staging", "deployed": True, "config": staging_config}
    ...
    async def generate_pipeline_report(self) -> Dict[str, Any]:
        ...
        passed_stages = sum(
            1 for stage in self.results.values() if stage["status"] == "PASSED"
        )
